...
<Callout type="info">
    This system was introduced during the 2026 REBUILT season and is now published as a **standalone reusable library** at
    `robot/robot-testing-utility/` in the mono-repo. Any robot project — including future seasons — can add it as a Gradle dependency.
    See **Installing the Package** below.
</Callout>
...
The RTU is a standalone Gradle library at `robot/robot-testing-utility/` in the mono-repo.
Add it to any robot project in two steps:

**`settings.gradle`** — declare the project path:
```groovy
...
<Callout type="tip">
    Don't want to use the mono-repo path? Download the prebuilt JAR from the **[GitHub Releases](https://github.com/feds201/feds-central/releases)** page
    (tags matching `rtu/v*`) and drop it in your project's `libs/` folder:
    ```groovy
...
import frc.rtu.DiagnosticContext; // only if you want alerts/profiling
```
...
All RTU core classes are in `robot/robot-testing-utility/src/main/java/frc/rtu/` (standalone package).
The `SubsystemStatusManager`, `DeviceTempReporter`, and `SwerveModuleStatusUtil` utilities remain in the robot project itself.

| File | Package | Class | Description |
|---|---|---|---|
| `RobotAction.java` | `frc.rtu` | `@RobotAction` | Annotation for marking test methods |
| `RootTestingUtility.java` | `frc.rtu` | `RootTestingUtility` | Discovery, execution, and reporting engine |
| `TestResult.java` | `frc.rtu` | `TestResult` | Immutable result record with alerts and data profiles |
| `DiagnosticContext.java` | `frc.rtu` | `DiagnosticContext` | Mutable context for recording alerts and samples |
| `DiagnosticServer.java` | `frc.rtu` | `DiagnosticServer` | Embedded HTTP dashboard with Chart.js visualization |
| `SubsystemStatusManager.java` | `frc.robot.utils` | `SubsystemStatusManager` | Continuous CAN device health monitoring via Shuffleboard |
| `DeviceTempReporter.java` | `frc.robot.utils` | `DeviceTempReporter` | Motor temperature tracking via Shuffleboard |
| `SwerveModuleStatusUtil.java` | `frc.robot.utils` | `SwerveModuleStatusUtil` | Swerve module registration convenience wrapper |
