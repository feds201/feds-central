...
2. `physicsSim.setInputVoltage(voltage)` — with inversion applied
3. `physicsSim.update(DT)`
4. `talonFXSimState.setRawRotorPosition(gearRatio * sim position)` — with inversion applied
5. `talonFXSimState.setRotorVelocity(gearRatio * sim velocity)` — with inversion applied
...
**No sim leaks in robot code:** The previous iteration leaked sim concepts into robot code (e.g. `AIMING_UP` / `AIMING_DOWN` enum values and `setSimPosition()` on `ShooterHood`). These have been deleted — sim positions are now driven entirely by the motor sim, and the hood state enum only carries real robot states.
...
### Tuned Physics Constants (2026)

After real-robot characterization passes, several constants were tuned away from placeholders:

| Constant | Value | Notes |
|----------|-------|-------|
| `SHOOTER_GEAR_RATIO` | `24/36` | Overdrive (motor spins faster than flywheel). |
| `HOOD_GEAR_RATIO` | derived | Computed from the motor soft-limit geometry (`HOOD_FORWARD_SOFT_LIMIT_ROT` across `HOOD_MIN_ANGLE_DEG`→`HOOD_MAX_ANGLE_DEG`) rather than a hand-picked number. |
| `INTAKE_DEPLOY_MOI` | `0.01` (was `0.0005`) | Raised to damp out intake deploy oscillation. |
| `SLIP_FACTOR` | `0.9` (was `0.8`) | Shooter wheel → ball velocity transfer. |
| `BALL_TRANSLATIONAL_FRACTION` | `0.55` (was `0.4`) | Fraction of wheel tangential speed imparted to the ball's translation. |
| `MAGNUS_COEFFICIENT` | `0.001` | Tuned for realistic ball flight curvature. |
| `BALL_ANGLE_OFFSET_DEG` | `15` | Extra launch pitch — the drum shooter exits steeper than the commanded hood angle. |
...
`ShooterSim` — the bridge that launches a game piece when the shooter fires — previously lived under `frc.robot.sim`. It has been moved to `robot/sim-core/src/main/java/frc/sim/gamepiece/ShooterSim.java` since nothing about it is game-specific; it still takes `DoubleSupplier`s for live hood angle and launch velocity so the year-specific layer wires it up with the 2026 shooter.
