...
<Callout type="info">
**2026 REBUILT Keyboard Controls**
...
Keys are grouped by row — each row maps to a part of the robot's ball path. Further right = more automation.

| Key | Xbox Equivalent | Action |
|-----|----------------|--------|
| **Driving (left side)** | | |
| `W` / `S` | Left Stick Y | Drive forward / back |
| `A` / `D` | Left Stick X | Strafe left / right |
| `Q` / `E` | Right Stick X | Rotate left / right |
| `X` | Start | Reset field-centric heading |
| **Intake (bottom row — `, . /`)** | | |
...
| **Shooting (top row — `P O [ ]`)** | | |
| `P` (hold) | X | Layup — shoot from against hub |
| `O` (hold) | Y | Halfcourt / passing shot |
| `[` (hold) | D-pad Right | Aim hub + spin up shooter |
| `]` (hold) | Right Trigger | Gated fire (only when aimed + at speed; use with `[`) |

</Callout>

<Callout type="info">
**2026 REBUILT Xbox Controls (Driver — port 0)**

| Button | Action |
|--------|--------|
| `Left Stick` | Drive (field-relative — forward/strafe) |
| `Right Stick X` | Rotate left / right |
| `Start` | Reset field-centric heading |
| `D-pad Right` | Hub aim + shooter spin-up (hold; fires when out of neutral zone) or passing shot aim (hold; fires when in neutral zone) |
| `Left Trigger` | Extend intake + run intake rollers (hold) |
| `Left Bumper` | Retract intake |
| `Right Bumper` | Reverse intake rollers |
| `Right Trigger` | Fire (gated — only fires when swerve is aimed at hub AND shooter at speed; use with D-pad Right) |
| `B` | Reverse feeder + spindexer (hold) |
| `X` | Shoot from hub (feeder + spindexer + shooter wheels + hood, hold) |
| `Y` | Halfcourt/passing shot (hold) |
