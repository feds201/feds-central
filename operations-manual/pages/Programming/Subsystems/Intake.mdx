...
- **`edu.wpi.first.math.controller.PIDController`**: A class used to implement a PID (Proportional, Integral, Derivative) control loop, which helps the robot move the wrist to a desired position smoothly and accurately.
...
- **`pid`**: A `PIDController` used to control the wrist's rotation. It helps adjust the motor's power to reach and maintain a specific target angle.
...
```
- **`periodic()`**: This method is called periodically by the robot framework (typically every 20 milliseconds).
  - **`wristRotationEncoder.setPositionOffset(0)`**: Resets the encoder position offset to zero during every periodic call, potentially preventing encoder drift.
