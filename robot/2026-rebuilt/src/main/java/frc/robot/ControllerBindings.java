...
/**
 * Helper that wires controller buttons to subsystem commands. Dependencies (controllers and
 * subsystems) are obtained from RobotContainer getters so both classes share the same subsystem
 * instances.
 */
public class ControllerBindings {
  ...
  public void setupTestBindings(CommandXboxController test) {
    ...
    test.povDown().whileTrue(shooterHood.setMotorPower(-0.05))
    ...
    test.y()
        .onTrue(
            feederSubsystem.commandRun().andThen(spinDexer.setStateCommand(spindexer_state.RUN)))
        .onFalse(
            feederSubsystem.commandStop().andThen(spinDexer.setStateCommand(spindexer_state.STOP)));
    test.a().onTrue(Commands.parallel(shooterWheels.setStateCommand(shooter_state.IDLE),
        feederSubsystem.setStateCommand(feeder_state.STOP)));

    test.b().onTrue(intakeSubsystem.setIntakeStateCommand(IntakeState.EXTENDED))
  ...
  public void setupDriveBindings(CommandXboxController driver) {
    ...
    driver.start().onTrue(new InstantCommand(drivetrain::seedFieldCentric));

    // -------- INTAKE CONTROLS ---------
    driver.leftTrigger().onTrue(intakeSubsystem.setIntakeStateCommand(IntakeState.INTAKING))
        .onFalse(intakeSubsystem.setRollerStateCommand(RollerState.OFF));

    driver.leftBumper().onTrue(intakeSubsystem.setIntakeStateCommand(IntakeState.DEFAULT));
    ...
    driver.b()
        .onTrue(feederSubsystem.setStateCommand(feeder_state.PREVERSE)
            .andThen(spinDexer.setStateCommand(spindexer_state.PREVERSE)))
        .onFalse(feederSubsystem.setStateCommand(feeder_state.STOP)
            .andThen(spinDexer.setStateCommand(spindexer_state.STOP)));
    driver.b()
        .onTrue(feederSubsystem.setStateCommand(feeder_state.PREVERSE)
            .andThen(spinDexer.setStateCommand(spindexer_state.PREVERSE)))
        .onFalse(feederSubsystem.setStateCommand(feeder_state.STOP)
    ...
    driver.rightBumper().whileTrue(intakeSubsystem.setRollerStateCommand(RollerState.REVERSE))
        .onFalse(intakeSubsystem.setRollerStateCommand(RollerState.OFF));

    driver.a()
        .onTrue(Commands
            .sequence(shooterHood.setStateCommand(shooterhood_state.HALFCOURT),
    ...
    // Button to shoot from against hub — run shooter/feeder/spindexer + agitate in parallel
    // while held
    driver.x()
    ...
    driver.y()
        .onTrue(Commands
            .sequence(shooterHood.setStateCommand(shooterhood_state.LAYUP),
    ...
    driver.povRight().and(
        () -> !ShooterConstants.neutralZone.contains(drivetrain.getState().Pose.getTranslation()))
        .and(() -> !drivetrain.inOtherAlliance())
        .whileTrue(Commands
            .sequence(shooterHood.setStateCommand(shooterhood_state.SHOOTING),
                shooterWheels.setStateCommand(shooter_state.SHOOTING))
            .alongWith(new HubDrive(drivetrain, driver)))
        .onFalse(Commands.sequence(shooterHood.setStateCommand(shooterhood_state.IN),
            shooterWheels.setStateCommand(shooter_state.IDLE)));

    // If in neutral zone, face outpost and ready shoot (passing shot)
    driver.povRight().and(() -> drivetrain.inNeutralZoneOrOpposing())
        .whileTrue(Commands
            .sequence(shooterHood.setStateCommand(shooterhood_state.PASSING),
  ...
  public void setupOperatorBindings(CommandXboxController operator) {
    ...
    operator.leftTrigger().onTrue(shooterHood.setMotorPower(0.1))
        .onFalse(shooterHood.setMotorPower(0.0));
    operator.leftBumper().onTrue(shooterHood.setMotorPower(-0.1))
        .onFalse(shooterHood.setMotorPower(0.0));
    ...
    operator.start()
        ...
        .onFalse(feederSubsystem.setStateCommand(feeder_state.STOP)
            .alongWith(spindexerSubsystem.setStateCommand(spindexer_state.STOP)));

    // Add multiplier to hood angle
    operator.povUp().onTrue(new InstantCommand(() -> shooterHood.updateHoodAngleMultiplier(.01)));
    operator.povDown()
    ...
    operator.a()
        .onTrue(intakeSubsystem.setIntakeStateCommand(IntakeState.CLOSE_AGITATION_OUT)
            .alongWith(intakeSubsystem.setRollerStateCommand(RollerState.AGITATEON)))
        .onFalse(intakeSubsystem.setIntakeStateCommand(IntakeState.INTAKING));

    operator.b()
        .onTrue(intakeSubsystem.setIntakeStateCommand(IntakeState.FAR_AGITATION_IN)
            .alongWith(intakeSubsystem.setRollerStateCommand(RollerState.AGITATEON)))
        .onFalse(intakeSubsystem.setIntakeStateCommand(IntakeState.INTAKING));
