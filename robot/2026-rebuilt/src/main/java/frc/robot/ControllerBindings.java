...
public class ControllerBindings {
  ...
  public void setupTestBindings(CommandXboxController test) {
    ...
    test.y()
        .onTrue(feederSubsystem.commandRun()
            .andThen(spinDexer.setStateCommand(spindexer_state.RUN)))
        .onFalse(feederSubsystem.commandStop()
            .andThen(spinDexer.setStateCommand(spindexer_state.STOP)));
    test.a().onTrue(
        Commands.parallel(shooterWheels.setStateCommand(shooter_state.IDLE),
            feederSubsystem.setStateCommand(feeder_state.STOP)));
  ...
  public void setupDriveBindings(CommandXboxController driver) {
    ...
    driver.povRight()
        .and(() -> !ShooterConstants.neutralZone
            .contains(drivetrain.getState().Pose.getTranslation()))
        ...
        .onFalse(
            Commands.sequence(shooterHood.setStateCommand(shooterhood_state.IN),
                shooterWheels.setStateCommand(shooter_state.IDLE)));
    ...
    driver.rightTrigger().and(HubDrive::pidAtSetpoint)
        .and(shooterWheels::atSetpoint)
        .whileTrue(Commands.sequence(
            feederSubsystem.setStateCommand(feeder_state.PRUN),
            spinDexer.setStateCommand(spindexer_state.PFORWARD),
            intakeSubsystem.setRollerStateCommand(RollerState.AGITATEON)))
  ...
  public void setupOperatorBindings(CommandXboxController operator) {
    ...
    operator.start()
        .onTrue(
            feederSubsystem.setStateCommand(feeder_state.PREVERSE).alongWith(
                spindexerSubsystem.setStateCommand(spindexer_state.PREVERSE)))
        .onFalse(feederSubsystem.setStateCommand(feeder_state.STOP).alongWith(
            spindexerSubsystem.setStateCommand(spindexer_state.STOP)));

    // Add multiplier to hood angle
    operator.povUp().onTrue(
        new InstantCommand(() -> shooterHood.updateHoodAngleMultiplier(.01)));
    operator.povDown().onTrue(
        new InstantCommand(() -> shooterHood.updateHoodAngleMultiplier(-.01)));

    operator.y().onTrue(
        intakeSubsystem.setIntakeStateCommand(IntakeState.AGITATE_IN).alongWith(
            intakeSubsystem.setRollerStateCommand(RollerState.AGITATEON)))
        .onFalse(intakeSubsystem.setIntakeStateCommand(IntakeState.INTAKING));

    operator.a()
        .onTrue(intakeSubsystem
            .setIntakeStateCommand(IntakeState.CLOSE_AGITATION_OUT).alongWith(
                intakeSubsystem.setRollerStateCommand(RollerState.AGITATEON)))
        .onFalse(intakeSubsystem.setIntakeStateCommand(IntakeState.INTAKING));

    operator.b()
        .onTrue(
            intakeSubsystem.setIntakeStateCommand(IntakeState.FAR_AGITATION_IN)
                .alongWith(intakeSubsystem
                    .setRollerStateCommand(RollerState.AGITATEON)))
        .onFalse(intakeSubsystem.setIntakeStateCommand(IntakeState.INTAKING));

    operator.x().onTrue(intakeSubsystem
