...
public final class RootTestingUtility {
  ...
  private record DiscoveredAction(SubsystemBase subsystem, Method method,
  ...
  /**
   * Scans every registered subsystem for methods carrying {@link RobotAction} and stores them sorted
   * by {@link RobotAction#order()}.
   *
   * <p>
  ...
  public void discoverActions() {
    ...
    for (SubsystemBase subsystem : registeredSubsystems) {
      ...
      for (Method m : clazz.getDeclaredMethods()) {
        ...
        if (ret != boolean.class && ret != Boolean.class && ret != void.class) {
          System.err.println("[RootTestingUtility] Skipping "
              + clazz.getSimpleName() + "." + m.getName()
              + " -- return type must be boolean or void, got "
              + ret.getSimpleName());
          continue;
        }
        ...
        if (params.length > 1) {
          System.err.println("[RootTestingUtility] Skipping "
              + clazz.getSimpleName() + "." + m.getName()
              + " -- expected 0 or 1 (DiagnosticContext) params, got "
              + params.length);
          continue;
        }
        if (params.length == 1 && params[0] != DiagnosticContext.class) {
          System.err.println(
              "[RootTestingUtility] Skipping " + clazz.getSimpleName() + "."
    ...
    actions.sort(
        Comparator.<DiscoveredAction>comparingInt(a -> a.annotation().order())
            .thenComparing(a -> resolveName(a)));

    System.out.println("[RootTestingUtility] Discovered " + actions.size()
        + " @RobotAction(s) across " + registeredSubsystems.size()
        + " subsystem(s):");
    for (DiscoveredAction a : actions) {
      System.out
          .println("  - " + a.subsystem().getName() + " -> " + resolveName(a));
    }
  }
  ...
  public void runAll() {
    ...
    new Thread(() -> {
      ...
      if (preStartDelayMs > 0) {
        ...
        System.out.println("[RootTestingUtility] Waiting "
            + (preStartDelayMs / 1000) + "s before starting tests...");
        while (System.currentTimeMillis() < waitEnd) {
          long remainingMs = waitEnd - System.currentTimeMillis();
          long remainingSec = Math.max(0, (remainingMs + 999) / 1000);
          diagServer
              .setSafetyMessage("Starting tests in " + remainingSec + "s");
          try {
            Thread.sleep(200);
          } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
  ...
  /**
   * Execute a single discovered action with timeout and optional {@link DiagnosticContext} injection.
   */
  private TestResult executeAction(DiscoveredAction action) {
    ...
    try {
      ...
      if (action.method().getReturnType() == void.class) {
        return new TestResult(subsystemName, actionName, description,
            TestResult.Status.PASSED, null, durationMs, ctx.getAlerts(),
            ctx.getDataProfiles());
      }
      ...
      return new TestResult(subsystemName, actionName, description,
          ...
          passed ? null : new AssertionError("Returned false"), durationMs,
      ...
      return new TestResult(subsystemName, actionName, description,
          TestResult.Status.FAILED, cause, durationMs, ctx.getAlerts(),
          ctx.getDataProfiles());
    }
  }
  ...
  private void publishResults() {
    ...
    Logger.recordOutput(LOG_PREFIX + "Summary/PassedList",
        passedNames.toArray(new String[0]));
    Logger.recordOutput(LOG_PREFIX + "Summary/FailedList",
        failedNames.toArray(new String[0]));

    if (diagServer != null) {
      Logger.recordOutput(LOG_PREFIX + "Summary/DashboardUrl",
          diagServer.getUrl());
    }
  }
