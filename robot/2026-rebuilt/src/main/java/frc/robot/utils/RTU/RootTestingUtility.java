 ...
 *
 * Automatically discovers methods annotated with {@link RobotAction} on any {@link SubsystemBase}
 * instance, then executes them in order when the robot enters <b>Test mode</b>.
 *
 * <h3>Lifecycle</h3>
 * <ol>
 * <li>{@link #registerSubsystem(SubsystemBase...)} -- call once from {@code RobotContainer} to hand
 * the utility every subsystem you want tested.</li>
 * <li>{@link #discoverActions()} -- called internally; scans registered subsystems for
 * {@code @RobotAction} methods via reflection.</li>
 * <li>{@link #runAll()} -- invoked from {@code Robot.testInit()} to run every discovered action and
 * record results.</li>
 * <li>{@link #periodic()} -- invoked from {@code Robot.testPeriodic()} to keep logged data
 * fresh.</li>
 * </ol>
 *
 * <h3>Result recording</h3> Results are published to:
 * <ul>
 * <li><b>Console</b> (System.out)</li>
 * <li><b>AdvantageKit Logger</b> -- under {@code RootTests/...} keys, viewable in
 * AdvantageScope</li>
 * <li><b>Diagnostic Server</b> -- embedded HTTP dashboard at
 * {@code http://&lt;ip&gt;:5800/diag/&lt;session&gt;} with charts, alerts, and data profiles</li>
 * </ul>
 *
 * <h3>DiagnosticContext injection</h3> If a {@code @RobotAction} method declares a single
 * {@link DiagnosticContext} parameter, the utility will automatically create one and pass it in.
 * The test can then call {@code ctx.info(...)}, {@code ctx.warn(...)}, {@code ctx.error(...)}, and
 * {@code ctx.sample(series, value)} to record alerts and profiling data.
 */
public final class RootTestingUtility {

  /** A single discovered action (subsystem instance + reflective method + annotation metadata). */
  private record DiscoveredAction(SubsystemBase subsystem, Method method, RobotAction annotation) {}
  ...
  /**
   * Register one or more subsystems whose {@code @RobotAction} methods should be discovered and
   * executed during test mode.
   */
  ...
  /**
   * Scans every registered subsystem for methods carrying {@link RobotAction} and stores them
   * sorted by {@link RobotAction#order()}.
   *
   * <p>
   * Methods may have zero parameters, or a single {@link DiagnosticContext} parameter. Any other
    ...
    for (SubsystemBase subsystem : registeredSubsystems) {
      ...
      for (Method m : clazz.getDeclaredMethods()) {
        ...
        if (ret != boolean.class && ret != Boolean.class && ret != void.class) {
          System.err
              .println("[RootTestingUtility] Skipping " + clazz.getSimpleName() + "." + m.getName()
                  + " -- return type must be boolean or void, got " + ret.getSimpleName());
          continue;
        }
        ...
        if (params.length > 1) {
          System.err
              .println("[RootTestingUtility] Skipping " + clazz.getSimpleName() + "." + m.getName()
                  + " -- expected 0 or 1 (DiagnosticContext) params, got " + params.length);
          continue;
        }
        if (params.length == 1 && params[0] != DiagnosticContext.class) {
          System.err
              .println("[RootTestingUtility] Skipping " + clazz.getSimpleName() + "." + m.getName()
                  + " -- param must be DiagnosticContext, got " + params[0].getSimpleName());
          continue;
        }
  ...
  /**
   * Runs every discovered action sequentially in a background thread, records {@link TestResult}s,
   * publishes to AK Logger, and updates the diagnostic server.
   */
  public void runAll() {
    if (actions.isEmpty()) {
      discoverActions();
    ...
    new Thread(() -> {
      ...
      if (preStartDelayMs > 0) {
        ...
        System.out.println("[RootTestingUtility] Waiting " + (preStartDelayMs / 1000)
            + "s before starting tests...");
      ...
      System.out.println("+==========================================+");
      System.out.printf("|  TOTAL: %d  |  PASSED: %d  |  FAILED: %d%n", results.size(), passed,
          failed);
      System.out.println("+==========================================+");
  ...
  /**
   * Execute a single discovered action with timeout and optional {@link DiagnosticContext}
   * injection.
   */
  private TestResult executeAction(DiscoveredAction action) {
    ...
    try {
      ...
      if (!finished) {
        ...
        return new TestResult(subsystemName, actionName, description, TestResult.Status.TIMED_OUT,
            new RuntimeException("Timed out after " + timeoutMs + " ms"), durationMs,
      ...
      if (action.method().getReturnType() == void.class) {
        return new TestResult(subsystemName, actionName, description, TestResult.Status.PASSED,
            null, durationMs, ctx.getAlerts(), ctx.getDataProfiles());
      }
      ...
      return new TestResult(subsystemName, actionName, description,
          ...
          passed ? null : new AssertionError("Returned false"), durationMs, ctx.getAlerts(),
          ctx.getDataProfiles());

    } catch (Exception e) {
      double durationMs = (System.nanoTime() - startNs) / 1_000_000.0;
      Throwable cause = e.getCause() != null ? e.getCause() : e;
      return new TestResult(subsystemName, actionName, description, TestResult.Status.FAILED, cause,
  ...
  private void publishResults() {
    ...
    for (TestResult r : results) {
      ...
      for (var entry : r.getDataProfiles().entrySet()) {
        ...
        double[] values =
            entry.getValue().stream().mapToDouble(TestResult.DataSample::value).toArray();
  ...
  /**
   * Call from {@code Robot.testPeriodic()} to keep AK Logger and diagnostic server data fresh.
   */
