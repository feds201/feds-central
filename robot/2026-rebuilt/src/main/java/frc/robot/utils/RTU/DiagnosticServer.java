...
public final class DiagnosticServer {
  ...
  private final List<CollectedSample> collectedSamples =
  ...
  private static class CollectedSample {
    ...
    CollectedSample(long t, double velocityRps, double hoodDeg,
        double distanceM) {
      this.t = t;
    ...
    CollectedSample(long t, double velocityRps, double hoodDeg,
        double distanceM, int successRating, String notes) {
      this.t = t;
      this.velocityRps = velocityRps;
      this.hoodDeg = hoodDeg;
      this.distanceM = distanceM;
  ...
  private static class SweepCycle {
    ...
    void addTrial(double velocityRps, double hoodDeg, double distanceM) {
      trials.add(new CollectedSample(System.currentTimeMillis(), velocityRps,
          hoodDeg, distanceM));
    }
  ...
  public void start() {
    try {
      ...
      server.createContext("/diag/" + sessionId + "/collector/hit",
          exchange -> {
            ...
            double v =
                frc.robot.utils.RTU.TelemetryPublisher.getShooterVelocityRps();
            double h = frc.robot.utils.RTU.TelemetryPublisher
                .getHoodPositionRotations();
            double d =
                frc.robot.utils.RTU.TelemetryPublisher.getDistanceToHubM();
            CollectedSample s =
                new CollectedSample(System.currentTimeMillis(), v, h, d);
      ...
      server.createContext("/diag/" + sessionId + "/collector/list",
          exchange -> {
            ...
            synchronized (collectedSamples) {
              for (int i = 0; i < collectedSamples.size(); i++) {
                ...
                jb.append(String.format(
                    "{\"t\":%d,\"v\":%.4f,\"h\":%.3f,\"d\":%.3f,\"r\":%d,\"notes\":\"%s\"}",
                    s.t, s.velocityRps, s.hoodDeg, s.distanceM, s.successRating,
                    jsonEsc(s.notes)));
              }
            }
            jb.append("]");
      ...
      server.createContext("/diag/" + sessionId + "/collector/csv",
          exchange -> {
            StringBuilder cb = new StringBuilder();
            cb.append(
                "timestamp_ms,shooter_rps,hood_deg,distance_m,success_rating,notes\n");
            synchronized (collectedSamples) {
              for (CollectedSample s : collectedSamples) {
                cb.append(String.format("%d,%.6f,%.4f,%.4f,%d,\"%s\"\n", s.t,
                    s.velocityRps, s.hoodDeg, s.distanceM, s.successRating,
                    s.notes.replace("\"", "\"\"")));
              }
            }
            ...
            exchange.getResponseHeaders().set("Content-Disposition",
                "attachment; filename=shooter_tuning_data.csv");
            exchange.sendResponseHeaders(200, bytes.length);
            try (OutputStream os = exchange.getResponseBody()) {
              os.write(bytes);
            }
      ...
      server.createContext("/diag/" + sessionId + "/collector/set",
          exchange -> {
            try {
      ...
      server.createContext("/diag/" + sessionId + "/collector/auto",
          exchange -> {
            try {
              var q = exchange.getRequestURI().getQuery();
              ...
              var rc = frc.robot.RobotContainer.getInstance();
              if (rc != null && !Double.isNaN(min) && !Double.isNaN(max)
                  && !Double.isNaN(step)) {
                rc.startAutoSweep(min, max, step,
                    Double.isNaN(hoodDeg) ? 0.0 : hoodDeg, holdMs);
      ...
      // Collector: auto stop
      server.createContext("/diag/" + sessionId + "/collector/auto/stop",
          exchange -> {
            var rc = frc.robot.RobotContainer.getInstance();
            if (rc != null) {
              rc.stopAutoSweep();
      ...
      server.createContext("/diag/" + sessionId + "/collector/auto/status",
          exchange -> {
            var rc = frc.robot.RobotContainer.getInstance();
      ...
      server.createContext("/diag/" + sessionId + "/tuning/save-sample",
          exchange -> {
            if (!"POST".equals(exchange.getRequestMethod())) {
              exchange.sendResponseHeaders(405, -1);
              return;
            ...
            try {
              var q = exchange.getRequestURI().getQuery();
              double velocity = frc.robot.utils.RTU.TelemetryPublisher
                  .getShooterVelocityRps();
              double hood = frc.robot.utils.RTU.TelemetryPublisher
                  .getHoodPositionRotations();
              double distance =
                  frc.robot.utils.RTU.TelemetryPublisher.getDistanceToHubM();
              ...
              CollectedSample s =
                  new CollectedSample(System.currentTimeMillis(), velocity,
                      hood, distance, rating, notes);
              synchronized (collectedSamples) {
                collectedSamples.add(s);
              }

              String resp = String.format(
                  "{\"ok\":true,\"saved\":{\"v\":%.4f,\"h\":%.3f,\"d\":%.3f,\"r\":%d}}",
      ...
      server.createContext("/diag/" + sessionId + "/tuning/predict",
          exchange -> {
            try {
              var q = exchange.getRequestURI().getQuery();
              ...
              if (q != null) {
                for (String part : q.split("&")) {
                  ...
                  if (kv.length == 2 && kv[0].equals("distance")) {
                    try {
                      targetDistance = Double.parseDouble(java.net.URLDecoder
                          .decode(kv[1], StandardCharsets.UTF_8));
                    } catch (NumberFormatException nfe) {
                    }
                  }
                }
              ...
              String resp = String.format(
                  ...
                  targetDistance, prediction[0], prediction[1], prediction[2],
                  prediction[3]);
      ...
      server.createContext("/diag/" + sessionId + "/tuning/cycle/start",
          exchange -> {
            try {
              var q = exchange.getRequestURI().getQuery();
              ...
              synchronized (this) {
                ...
                for (int i = 0; i < 5; i++) {
                  ...
                  currentCycle.addTrial(baseVelocity + vOffset,
                      baseHood + hOffset, distance);
                }
              }

              String resp = String.format(
                  "{\"started\":true,\"trialCount\":%d,\"baseVelocity\":%.4f,\"baseHood\":%.3f}",
      ...
      server.createContext("/diag/" + sessionId + "/tuning/cycle/status",
          exchange -> {
            ...
            synchronized (this) {
              if (currentCycle == null) {
                ...
                resp.append("{\"active\":true,\"completed\":")
                    .append(currentCycle.completed);
                resp.append(",\"trialCount\":")
                    .append(currentCycle.trials.size());
                resp.append(",\"trials\":[");
                for (int i = 0; i < currentCycle.trials.size(); i++) {
                  ...
                  resp.append(String.format(
                      "{\"index\":%d,\"velocity\":%.4f,\"hood\":%.3f,\"distance\":%.3f}",
      ...
      server.createContext("/diag/" + sessionId + "/tuning/cycle/complete",
          exchange -> {
            if (!"POST".equals(exchange.getRequestMethod())) {
              exchange.sendResponseHeaders(405, -1);
              return;
            ...
            try {
              ...
              synchronized (this) {
                if (currentCycle != null && bestTrial >= 0
                    && bestTrial < currentCycle.trials.size()) {
                  ...
                  CollectedSample ratedSample =
                      new CollectedSample(System.currentTimeMillis(),
                          best.velocityRps, best.hoodDeg, best.distanceM,
                          rating, "Auto-cycle best trial");
      ...
      System.err.println("[DiagnosticServer] Failed to start on port " + port
          + ": " + e.getMessage());
    }
  }
  ...
  private synchronized String buildHtml() {
    ...
    sb.append("<p class='session'>Session: <code>").append(sessionId)
        .append("</code></p>");
    ...
    sb.append(
        "<p>Comprehensive tuning tool with manual controls, prediction, and automated optimization.</p>");
    ...
    sb.append(
        "<div class='telemetry-item'><label>Shooter RPS:</label><span id='tel_shooter' class='telemetry-value'>--</span></div>");
    sb.append(
        "<div class='telemetry-item'><label>Hood Position:</label><span id='tel_hood' class='telemetry-value'>-- rot</span></div>");
    sb.append(
        "<div class='telemetry-item'><label>Distance:</label><span id='tel_dist' class='telemetry-value'>-- m</span></div>");
    sb.append("</div></div>");
    ...
    sb.append(
        "<input type='range' id='velocity-slider' min='0' max='100' value='30' step='0.1' oninput='updateVelocityDisplay()'>");
    sb.append("<button onclick='adjustVelocity(1)'>+</button>");
    sb.append(
        "<input type='number' id='velocity-input' value='30' min='0' max='100' step='0.1' onchange='syncVelocitySlider()'>");
    sb.append("</div></div>");

    // Hood angle controls
    sb.append("<div class='control-group'>");
    sb.append(
        "<label class='control-label'>Hood Position (rotations):</label>");
    sb.append("<div class='slider-control'>");
    sb.append("<button onclick='adjustHood(-0.5)'>-</button>");
    sb.append(
        "<input type='range' id='hood-slider' min='0' max='30' value='15' step='0.1' oninput='updateHoodDisplay()'>");
    sb.append("<button onclick='adjustHood(0.5)'>+</button>");
    sb.append(
        "<input type='number' id='hood-input' value='15' min='0' max='30' step='0.1' onchange='syncHoodSlider()'>");
    sb.append("</div></div>");

    // Action buttons
    sb.append("<div class='action-buttons'>");
    sb.append(
        "<button id='btn-apply' class='btn-primary' onclick='applySettings()'>Apply Settings</button>");
    sb.append(
        "<button id='btn-fire' class='btn-fire' onclick='fireShooter()'>🔥 FIRE</button>");
    sb.append("<div class='save-controls'>");
    sb.append("<select id='success-rating'>");
    sb.append(
        "<option value='0'>0 - Miss</option><option value='1'>1 - Very Poor</option><option value='2'>2 - Poor</option>");
    sb.append(
        "<option value='3' selected>3 - OK</option><option value='4'>4 - Good</option><option value='5'>5 - Perfect</option>");
    sb.append("</select>");
    sb.append(
        "<input type='text' id='shot-notes' placeholder='Shot notes...' maxlength='100'>");
    sb.append(
        "<button id='btn-save' class='btn-success' onclick='saveSample()'>Save Sample</button>");
    sb.append("</div></div>");
    sb.append("</div>");
    ...
    sb.append(
        "<input type='number' id='target-distance' value='2.5' min='1' max='10' step='0.1'>");
    ...
    sb.append(
        "<label>Base Velocity:</label><input type='number' id='base-velocity' value='30' step='1'>");
    sb.append(
        "<label>Base Hood:</label><input type='number' id='base-hood' value='15' step='0.5'>");
    sb.append("</div>");
    sb.append("<div class='input-row'>");
    sb.append(
        "<label>Velocity Range:</label><input type='number' id='velocity-range' value='10' step='1'>");
    sb.append(
        "<label>Hood Range:</label><input type='number' id='hood-range' value='5' step='0.5'>");
    sb.append(
        "<label>Distance:</label><input type='number' id='cycle-distance' value='2.5' step='0.1'>");
    sb.append("</div>");
    ...
    sb.append("<a href='/diag/").append(sessionId).append(
        "/collector/csv' target='_blank'><button>Download CSV</button></a>");
    ...
    sb.append(
        "<thead><tr><th>Time</th><th>Velocity</th><th>Hood</th><th>Distance</th><th>Rating</th><th>Notes</th></tr></thead>");
    ...
    if (latestResults.isEmpty()) {
      ...
      sb.append(
          "<p class='waiting'>No RTU test results yet. The shooter tuning system above is ready to use!</p>");
      sb.append("</div>");
    } else {
      ...
      sb.append("<span>Total: <b>").append(latestResults.size())
          .append("</b></span>");
      ...
      sb.append(
          "      document.getElementById('tel_shooter').textContent=j.shooterRps.toFixed(3);\n");
      sb.append(
          "      document.getElementById('tel_hood').textContent=j.hoodRot.toFixed(2)+' rot';\n");
      sb.append(
          "      document.getElementById('tel_dist').textContent=j.distM.toFixed(2)+' m';\n");
      sb.append("    }\n");
      sb.append("  }catch(e){}\n");
      ...
      sb.append(
          "  const val=Math.max(0,Math.min(100,parseFloat(input.value)+delta));\n");
      ...
      sb.append(
          "  const val=Math.max(0,Math.min(30,parseFloat(input.value)+delta));\n");
      sb.append("  input.value=val.toFixed(1);\n");
      ...
      sb.append(
          "  document.getElementById('velocity-input').value=parseFloat(slider.value).toFixed(1);\n");
      sb.append("}\n");

      sb.append("function updateHoodDisplay(){\n");
      sb.append("  const slider=document.getElementById('hood-slider');\n");
      sb.append(
          "  document.getElementById('hood-input').value=parseFloat(slider.value).toFixed(1);\n");
      sb.append("}\n");

      sb.append("function syncVelocitySlider(){\n");
      sb.append("  const input=document.getElementById('velocity-input');\n");
      sb.append(
          "  document.getElementById('velocity-slider').value=input.value;\n");
      sb.append("}\n");

      sb.append("function syncHoodSlider(){\n");
      sb.append("  const input=document.getElementById('hood-input');\n");
      sb.append(
          "  document.getElementById('hood-slider').value=input.value;\n");
      sb.append("}\n");
      ...
      sb.append("  try{\n");
      sb.append(
          "    const q=`?velocity=${encodeURIComponent(velocity)}&hoodDeg=${encodeURIComponent(hood)}`;\n");
      ...
      sb.append(
          "  showStatus('Fire command sent! (Robot controls actual firing)','info');\n");
      sb.append(
          "  // Note: Actual firing is controlled by robot operator/autonomous systems\n");
      sb.append("}\n");

      // Save sample with rating
      sb.append("async function saveSample(){\n");
      sb.append(
          "  const rating=document.getElementById('success-rating').value;\n");
      sb.append(
          "  const notes=encodeURIComponent(document.getElementById('shot-notes').value);\n");
      sb.append("  try{\n");
      sb.append("    const q=`?rating=${rating}&notes=${notes}`;\n");
      sb.append(
          "    const r=await fetch(base+'/tuning/save-sample'+q,{method:'POST'});\n");
      sb.append("    if(r.ok){\n");
      sb.append("      const result=await r.json();\n");
      sb.append(
          "      showStatus(`Sample saved with rating ${rating}/5`,'success');\n");
      ...
      sb.append(
          "  const distance=document.getElementById('target-distance').value;\n");
      sb.append("  try{\n");
      sb.append(
          "    const r=await fetch(base+'/tuning/predict?distance='+encodeURIComponent(distance));\n");
      sb.append("    if(r.ok){\n");
      ...
      sb.append(
          "          <div>Velocity: <strong>${pred.predictedVelocity.toFixed(2)} RPS</strong></div>\n");
      sb.append(
          "          <div>Hood Angle: <strong>${pred.predictedHood.toFixed(2)}°</strong></div>\n");
      sb.append(
          "          <div>Confidence: <strong>${(pred.confidence*100).toFixed(1)}%</strong> (${pred.sampleCount} samples)</div>\n");
      sb.append(
          "          <button onclick='applyPrediction(${pred.predictedVelocity},${pred.predictedHood})'>Apply This Prediction</button>\n");
      sb.append("        </div>`;\n");
      sb.append("    }else{\n");
      ...
      sb.append(
          "  document.getElementById('velocity-input').value=vel.toFixed(1);\n");
      sb.append(
          "  document.getElementById('hood-input').value=hood.toFixed(1);\n");
      sb.append("  syncVelocitySlider();\n");
      sb.append("  syncHoodSlider();\n");
      ...
      sb.append(
          "  const baseVel=document.getElementById('base-velocity').value;\n");
      sb.append(
          "  const baseHood=document.getElementById('base-hood').value;\n");
      sb.append(
          "  const velRange=document.getElementById('velocity-range').value;\n");
      sb.append(
          "  const hoodRange=document.getElementById('hood-range').value;\n");
      sb.append(
          "  const distance=document.getElementById('cycle-distance').value;\n");
      sb.append("  try{\n");
      sb.append(
          "    const q=`?baseVelocity=${baseVel}&baseHood=${baseHood}&velocityRange=${velRange}&hoodRange=${hoodRange}&distance=${distance}`;\n");
      sb.append("    const r=await fetch(base+'/tuning/cycle/start'+q);\n");
      sb.append("    if(r.ok){\n");
      sb.append("      cycleActive=true;\n");
      sb.append(
          "      showStatus('Cycle started! Ready for 5 trials.','success');\n");
      ...
      sb.append(
          "  let html='<h4>🔄 Trial Configuration:</h4><div class=\"trial-grid\">';\n");
      ...
      sb.append(
          "      <button onclick=\"applyTrial(${trial.velocity},${trial.hood})\">Use This</button>\n");
      sb.append("    </div>`;\n");
      sb.append("  });\n");
      sb.append("  html+='</div>';\n");
      sb.append(
          "  html+='<div class=\"cycle-completion\"><h4>After testing all trials:</h4>';\n");
      ...
      sb.append(
          "  html+='<label><input type=\"checkbox\" id=\"cycle-successful\"> Cycle was successful</label>';\n");
      sb.append(
          "  html+='<button onclick=\"completeCycle()\">Complete Cycle</button></div>';\n");
      ...
      sb.append(
          "  const bestTrial=document.getElementById('best-trial').value;\n");
      sb.append(
          "  const successful=document.getElementById('cycle-successful').checked;\n");
      sb.append("  try{\n");
      sb.append(
          "    const q=`?bestTrial=${bestTrial}&successful=${successful}`;\n");
      sb.append(
          "    const r=await fetch(base+'/tuning/cycle/complete'+q,{method:'POST'});\n");
      sb.append("    if(r.ok){\n");
      sb.append(
          "      showStatus(`Cycle completed! Best trial: ${parseInt(bestTrial)+1}`,'success');\n");
      sb.append("      cycleActive=false;\n");
      sb.append(
          "      document.getElementById('cycle-trials').innerHTML='';\n");
      sb.append("      refreshDataList();\n");
      sb.append("    }else{\n");
      ...
      sb.append(
          "    const tbody=document.querySelector('#samples_tbl tbody');\n");
      ...
      sb.append(
          "      tr.innerHTML=`<td>${time}</td><td>${s.v.toFixed(2)}</td><td>${s.h.toFixed(1)}°</td><td>${s.d.toFixed(2)}m</td><td>${rating}</td><td>${s.notes||''}</td>`;\n");
      sb.append("      tbody.appendChild(tr);\n");
      sb.append("    });\n");
      sb.append(
          "    const avgRating=ratedCount>0?(totalRating/ratedCount).toFixed(1):'–';\n");
      sb.append(
          "    document.getElementById('data-stats').textContent=`${samples.length} samples, ${ratedCount} rated (avg: ${avgRating}/5)`;\n");
      sb.append("  }catch(e){}\n");
      sb.append("}\n");
      ...
      if (latestResults.isEmpty()) {
        ...
        // Summary bar
        long passedRtu =
            latestResults.stream().filter(TestResult::isPassed).count();
        long failedRtu = latestResults.size() - passedRtu;
        String summaryClassRtu =
            failedRtu == 0 ? "summary-pass" : "summary-fail";
        sb.append("<div class='card'>");
        sb.append("<h2>🔧 Root Test Diagnostics Results</h2>");
        ...
        for (TestResult r : latestResults) {
          ...
          sb.append("<span class='status-badge ")
              .append(r.getStatus().name().toLowerCase()).append("'>");
          sb.append(r.getStatus().name()).append("</span>");
          sb.append("<h2>").append(esc(r.getActionName())).append("</h2>");
          sb.append("<span class='subsystem'>")
              .append(esc(r.getSubsystemName())).append("</span>");
          sb.append("</div>");

          sb.append("<p class='desc'>").append(esc(r.getDescription()))
              .append("</p>");
          sb.append("<p class='meta'>Duration: <b>")
              .append(String.format("%.2f", r.getDurationMs()));
          sb.append(" ms</b> | Timestamp: ")
              .append(TIME_FMT.format(r.getTimestamp())).append("</p>");
          ...
          if (!r.getDataProfiles().isEmpty()) {
            ...
            for (Map.Entry<String, List<DataSample>> entry : r.getDataProfiles()
                .entrySet()) {
              ...
              sb.append("<script>");
              sb.append("new Chart(document.getElementById('").append(canvasId)
                  .append("'),{");
              ...
              sb.append("],datasets:[{label:'").append(esc(entry.getKey()))
                  .append("',");
              sb.append("data:[");
              for (int i = 0; i < samples.size(); i++) {
                if (i > 0) {
  ...
  private synchronized String buildJson() {
    ...
    if (safetyMessage != null) {
      sb.append(",\"safetyMessage\":\"").append(jsonEsc(safetyMessage))
          .append("\"");
    }
    sb.append(",\"results\":[");
    for (int i = 0; i < latestResults.size(); i++) {
      ...
      sb.append("{\"subsystem\":\"").append(jsonEsc(r.getSubsystemName()))
          .append("\"");
      sb.append(",\"action\":\"").append(jsonEsc(r.getActionName()))
          .append("\"");
      sb.append(",\"status\":\"").append(r.getStatus().name()).append("\"");
      sb.append(",\"passed\":").append(r.isPassed());
      sb.append(",\"durationMs\":")
          .append(String.format("%.2f", r.getDurationMs()));
      sb.append(",\"description\":\"").append(jsonEsc(r.getDescription()))
          .append("\"");
      if (r.getError() != null) {
        sb.append(",\"error\":\"").append(jsonEsc(r.getError().getMessage()))
            .append("\"");
      }
      // Alerts
      sb.append(",\"alerts\":[");
      ...
      for (var entry : r.getDataProfiles().entrySet()) {
        ...
        for (int j = 0; j < entry.getValue().size(); j++) {
          ...
          sb.append(",\"v\":").append(String.format("%.4f", s.value()))
              .append("}");
        }
        sb.append("]");
      }
  ...
  // ── Utilities ────────────────────────────────────────────
  /**
   * Calculate predicted velocity and hood angle for a target distance using linear regression on the
   * collected samples with good ratings (3+).
   *
  ...
  private double[] calculatePrediction(double targetDistance) {
    ...
    synchronized (collectedSamples) {
      goodSamples = collectedSamples.stream().filter(s -> s.successRating >= 3) // Only use samples rated 3 or better
