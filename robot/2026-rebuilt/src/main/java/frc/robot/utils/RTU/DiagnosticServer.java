...
public final class DiagnosticServer {
  ...
  private static class CollectedSample {
    ...
    CollectedSample(long t, double velocityRps, double hoodDeg, double distanceM, int successRating,
        String notes) {
      this.t = t;
      this.velocityRps = velocityRps;
      this.hoodDeg = hoodDeg;
  ...
  public void start() {
    try {
      ...
      server.createContext("/diag/" + sessionId + "/telemetry", exchange -> {
        String json =
            String.format("{\"shooterRps\":%.4f,\"hoodDeg\":%.3f,\"hoodRot\":%.3f,\"distM\":%.3f}",
        ...
        synchronized (collectedSamples) {
          for (int i = 0; i < collectedSamples.size(); i++) {
            ...
            jb.append(String.format(
                "{\"t\":%d,\"v\":%.4f,\"h\":%.3f,\"d\":%.3f,\"r\":%d,\"notes\":\"%s\"}", s.t,
                s.velocityRps, s.hoodDeg, s.distanceM, s.successRating, jsonEsc(s.notes)));
          }
        }
        jb.append("]");
      ...
      server.createContext("/diag/" + sessionId + "/collector/csv", exchange -> {
        ...
        synchronized (collectedSamples) {
          for (CollectedSample s : collectedSamples) {
            cb.append(String.format("%d,%.6f,%.4f,%.4f,%d,\"%s\"\n", s.t, s.velocityRps, s.hoodDeg,
                s.distanceM, s.successRating, s.notes.replace("\"", "\"\"")));
          }
        }
      ...
      server.createContext("/diag/" + sessionId + "/tuning/save-sample", exchange -> {
        ...
        try {
          ...
          CollectedSample s = new CollectedSample(System.currentTimeMillis(), velocity, hood,
              distance, rating, notes);
          synchronized (collectedSamples) {
            collectedSamples.add(s);
          }

          String resp =
              String.format("{\"ok\":true,\"saved\":{\"v\":%.4f,\"h\":%.3f,\"d\":%.3f,\"r\":%d}}",
          ...
          if (q != null) {
            for (String part : q.split("&")) {
              ...
              if (kv.length == 2 && kv[0].equals("distance")) {
                try {
                  targetDistance =
                      Double.parseDouble(java.net.URLDecoder.decode(kv[1], StandardCharsets.UTF_8));
                } catch (NumberFormatException nfe) {
                }
              }
            }
      ...
      server.createContext("/diag/" + sessionId + "/tuning/cycle/start", exchange -> {
        try {
          ...
          String resp = String.format(
              "{\"started\":true,\"trialCount\":%d,\"baseVelocity\":%.4f,\"baseHood\":%.3f}", 5,
              baseVelocity, baseHood);
          byte[] bytes = resp.getBytes(StandardCharsets.UTF_8);
          exchange.getResponseHeaders().set("Content-Type", "application/json; charset=UTF-8");
          exchange.sendResponseHeaders(200, bytes.length);
          try (OutputStream os = exchange.getResponseBody()) {
        ...
        synchronized (this) {
          if (currentCycle == null) {
            ...
            for (int i = 0; i < currentCycle.trials.size(); i++) {
              ...
              resp.append(
                  String.format("{\"index\":%d,\"velocity\":%.4f,\"hood\":%.3f,\"distance\":%.3f}",
        ...
        try {
          ...
          synchronized (this) {
            if (currentCycle != null && bestTrial >= 0 && bestTrial < currentCycle.trials.size()) {
              ...
              CollectedSample ratedSample = new CollectedSample(System.currentTimeMillis(),
                  best.velocityRps, best.hoodDeg, best.distanceM, rating, "Auto-cycle best trial");
      ...
      System.err
          .println("[DiagnosticServer] Failed to start on port " + port + ": " + e.getMessage());
    }
  }
  ...
  private synchronized String buildHtml() {
    ...
    sb.append("<a href='/diag/").append(sessionId)
        .append("/collector/csv' target='_blank'><button>Download CSV</button></a>");
    ...
    if (latestResults.isEmpty()) {
      ...
      if (latestResults.isEmpty()) {
        ...
        for (TestResult r : latestResults) {
          ...
          sb.append("<span class='status-badge ").append(r.getStatus().name().toLowerCase())
              .append("'>");
          ...
          sb.append(" ms</b> | Timestamp: ").append(TIME_FMT.format(r.getTimestamp()))
              .append("</p>");
  ...
  // ── Utilities ────────────────────────────────────────────
  /**
   * Calculate predicted velocity and hood angle for a target distance using linear regression on
   * the collected samples with good ratings (3+).
   *
  ...
  private double[] calculatePrediction(double targetDistance) {
    ...
    synchronized (collectedSamples) {
      goodSamples = collectedSamples.stream().filter(s -> s.successRating >= 3) // Only use samples
                                                                                // rated 3 or better
