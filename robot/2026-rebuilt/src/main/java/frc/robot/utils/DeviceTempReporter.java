...
public class DeviceTempReporter {
  private static final ShuffleboardTab tab =
      Shuffleboard.getTab("Device Temp Reporter");
  private static final Map<String, Supplier<Temperature>> deviceTempSuppliers =
      new LinkedHashMap<>();
  private static final Map<String, GenericEntry> deviceTempEntries =
      new LinkedHashMap<>();
  ...
  /**
   * Method used to manually add individual devices (Recommended for non-CTRE devices such as
   * limelights)
   *
   * @param id String identifier for the device
   * @param temSupplier supplier to supply the temperature of the device
   */
  public static synchronized void addDevice(String id,
      Supplier<Temperature> tempSupplier) {
    if (id == null || tempSupplier == null) {
      throw new IllegalArgumentException(
          "id and tempSupplier must be non-null");
    }
  ...
  public static synchronized void pollAll() {
    for (Map.Entry<String, Supplier<Temperature>> e : deviceTempSuppliers
        .entrySet()) {
      ...
      try {
        ...
        System.err.println("[DeviceTempReporter] Exception while polling '"
            + name + "': " + t.getMessage());
      }
      GenericEntry entry = deviceTempEntries.get(name);
      if (entry != null)
        entry.setValue(temp.in(Fahrenheit));
    }
  }
}
