...
public class ShootOnTheMove {
   ...
   */
  public static Translation2d calculateVirtualGoal(Pose2d robotPose,
      ChassisSpeeds chassisSpeeds) {
    Translation2d hubCenter = RobotMap.ShooterConstants.hubCenter;
    if (DriverStation.getAlliance().orElse(Alliance.Blue) == Alliance.Red) {
      hubCenter =
          FlippingUtil.flipFieldPosition(RobotMap.ShooterConstants.hubCenter);
    }
    ...
    Translation2d shooterVelocity =
        getShooterFieldVelocity(robotPose, chassisSpeeds);
    ...
    for (int i = 0; i <= 10; i++) {
      ...
      double wheelVelocity =
          ShooterConstants.kShootingVelocityMap.get(distToGoal);
      ...
      double flightTime = -0.000772833 * Math.pow(wheelVelocity, 2)
          ...
          - 0.00218423 * Math.pow(hoodAngle, 2) - 0.136261 * hoodAngle
          - 0.149612;
    ...
    Logger.recordOutput("ShootOnTheMove",
        new Pose2d(hubCenter, new Rotation2d()));
   ...
   */
  public static Rotation2d calculateRobotHeading(Pose2d robotPose,
      ChassisSpeeds chassisSpeeds) {
    // Get shooter field position
    Translation2d shooterFieldPosition = getShooterFieldPosition(robotPose);
    ...
    Logger.recordOutput("VirtualGoal", new Pose2d(virtualGoal.getX(),
        virtualGoal.getY(), virtualGoal.getAngle()));


    Rotation2d shooterFieldAngle =
        new Rotation2d(shooterToGoal.getX(), shooterToGoal.getY());

    // Convert to required robot heading
    // θ_robot = θ_shooter_field - θ_RS
    return shooterFieldAngle
        .minus(RobotMap.ShooterConstants.robotToShooterRotation);
  }
  ...
  private static Translation2d getShooterFieldPosition(Pose2d robotPose) {
    ...
    Transform2d robotToShooter =
        new Transform2d(ShooterConstants.robotShooterOffset,
   ...
   * @return total velocity of rotational and translational of shooter relative to the field
   */
  private static Translation2d getShooterFieldVelocity(Pose2d robotPose,
    ...
    Translation2d rotationalVelocity = getRotationalVelocityContribution(
        robotPose.getRotation(), speeds.omegaRadiansPerSecond);
  ...
  private static Translation2d getRotationalVelocityContribution(
      Rotation2d robotHeading, double omega) {
    ...
    Translation2d robotFrameRotVel = new Translation2d(-omega * y, omega * x);
