...
public class PitTesting {
  ...
  private static SwerveRequest.FieldCentric driveRequest =
      new SwerveRequest.FieldCentric();


  private static ShuffleboardTab pitTab = Shuffleboard.getTab("Pit Testing");
  private static HashMap<String, GenericEntry> entryMap =
      new HashMap<String, GenericEntry>();
  private static ShuffleboardLayout testLayout =
      Shuffleboard.getTab("Pit Commands")
  ...
  public static void createDashboard() {
    ...
    entryMap.put("limit switch",
        pitTab.add("switch pressed", false).getEntry());
    ...
    testLayout.add("Run Testing", Commands.sequence(
        new InstantCommand(() -> Elastic.sendNotification(
            new Elastic.Notification(Elastic.NotificationLevel.ERROR,
                "Robot Enabling", "Clear the area"))),
        new InstantCommand(() -> testOrchestra.play()),
        Commands.waitSeconds(musicTime),
        new InstantCommand(() -> testOrchestra.stop()),
        testing.setStateCommand(MotorState.ON), Commands.waitSeconds(testTime),
    ...
    testLayout.add("test Drivetrain translation",
        Commands.sequence(
            ...
            Commands.waitSeconds(drivetrainTestTime), drivetrain.runOnce(
    ...
    testLayout.add("test Drivetrain rotation", Commands.sequence(
        ...
        new InstantCommand(() -> orchestra.stop()),
        drivetrain.runOnce(
            () -> drivetrain.setControl(driveRequest.withRotationalRate(1.0))),
        Commands.waitSeconds(drivetrainTestTime), drivetrain
            .runOnce(() -> drivetrain.setControl(new SwerveRequest.Idle()))));
  }
  ...
  private static void registerEntry(String name) {
    entryMap.put(name + " Connected",
        pitTab.add(name + " Connected", false).getEntry());
    entryMap.put(name + " Powered",
        pitTab.add(name + " Powered", false).getEntry());
  }
  ...
  private static void updateEntry(String name, TalonFX motor) {
    if (entryMap.get(name + " Connected") != null) {
      ...
      entryMap.get(name + " Powered").setBoolean(
          motor.getSupplyVoltage().getValueAsDouble() > poweredThreshold);
    }
  }
  ...
  private static void updateEntry(String name, CANcoder encoder) {
    if (entryMap.get(name + " Connected") != null) {
      ...
      entryMap.get(name + " Powered").setBoolean(
          encoder.getSupplyVoltage().getValueAsDouble() > poweredThreshold);
    }
  }
  ...
  private static void updateEntry(String name, Pigeon2 pigeon) {
    if (entryMap.get(name + " Connected") != null) {
      ...
      entryMap.get(name + " Powered").setBoolean(
          pigeon.getSupplyVoltage().getValueAsDouble() > poweredThreshold);
    }
  }
}
