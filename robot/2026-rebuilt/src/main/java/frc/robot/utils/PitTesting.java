...
public class PitTesting {
  ...
  private static ShuffleboardLayout testLayout = Shuffleboard.getTab("Pit Commands")
      .getLayout("Test Commands", BuiltInLayouts.kList).withSize(2, 5);
  ...
  public static void createDashboard() {
    ...
    // Commands.parallel(
    // new InstantCommand(() -> Elastic.sendNotification(new
    // Elastic.Notification(Elastic.NotificationLevel.ERROR, "Robot Enabling", "Clear the area"))),
    // Commands.sequence(
    ...
    // Add commands
    testLayout.add("Run Testing",
        Commands.sequence(
            new InstantCommand(() -> Elastic.sendNotification(new Elastic.Notification(
                Elastic.NotificationLevel.ERROR, "Robot Enabling", "Clear the area"))),
            new InstantCommand(() -> testOrchestra.play()), Commands.waitSeconds(musicTime),
            new InstantCommand(() -> testOrchestra.stop()), testing.setStateCommand(MotorState.ON),
            Commands.waitSeconds(testTime), testing.setStateCommand(MotorState.OFF)));

    testLayout.add("Run Feeder",
        Commands.sequence(
            new InstantCommand(() -> Elastic.sendNotification(new Elastic.Notification(
                Elastic.NotificationLevel.ERROR, "Robot Enabling", "Clear the area"))),
            new InstantCommand(() -> orchestra.play()), Commands.waitSeconds(musicTime),
            new InstantCommand(() -> orchestra.stop()), feeder.setStateCommand(feeder_state.RUN),
            Commands.waitSeconds(testTime), feeder.setStateCommand(feeder_state.STOP)));

    testLayout.add("Run Intake",
        Commands.sequence(
            new InstantCommand(() -> Elastic.sendNotification(new Elastic.Notification(
                Elastic.NotificationLevel.ERROR, "Robot Enabling", "Clear the area"))),
            new InstantCommand(() -> orchestra.play()), Commands.waitSeconds(musicTime),
            new InstantCommand(() -> orchestra.stop()),
            intake.setIntakeStateCommand(IntakeState.INTAKING), Commands.waitSeconds(testTime),
            intake.setIntakeStateCommand(IntakeState.DEFAULT)));

    testLayout.add("Run Intake and Rollers",
        Commands.sequence(
            new InstantCommand(() -> Elastic.sendNotification(new Elastic.Notification(
                Elastic.NotificationLevel.ERROR, "Robot Enabling", "Clear the area"))),
            new InstantCommand(() -> orchestra.play()), Commands.waitSeconds(musicTime),
            new InstantCommand(() -> orchestra.stop()),
            intake.setIntakeStateCommand(IntakeState.INTAKING), Commands.waitSeconds(testTime),
            intake.setRollerStateCommand(RollerState.AGITATEON), Commands.waitSeconds(testTime),
            intake.setRollerStateCommand(RollerState.OFF), Commands.waitSeconds(testTime),
            intake.setIntakeStateCommand(IntakeState.DEFAULT)));

    testLayout.add("Run Spindexer",
        Commands.sequence(
            new InstantCommand(() -> Elastic.sendNotification(new Elastic.Notification(
                Elastic.NotificationLevel.ERROR, "Robot Enabling", "Clear the area"))),
            new InstantCommand(() -> orchestra.play()), Commands.waitSeconds(musicTime),
            new InstantCommand(() -> orchestra.stop()),
            spindexer.setStateCommand(spindexer_state.RUN), Commands.waitSeconds(testTime),
            spindexer.setStateCommand(spindexer_state.STOP)));

    testLayout.add("Run Shooter Wheels",
        Commands.sequence(
            new InstantCommand(() -> Elastic.sendNotification(new Elastic.Notification(
                Elastic.NotificationLevel.ERROR, "Robot Enabling", "Clear the area"))),
            new InstantCommand(() -> orchestra.play()), Commands.waitSeconds(musicTime),
            new InstantCommand(() -> orchestra.stop()),
            shooterWheels.setStateCommand(shooter_state.SHOOTING), Commands.waitSeconds(testTime),
            shooterWheels.setStateCommand(shooter_state.IDLE)));

    testLayout.add("Move Shooter Hood",
        Commands.sequence(
            new InstantCommand(() -> Elastic.sendNotification(new Elastic.Notification(
                Elastic.NotificationLevel.ERROR, "Robot Enabling", "Clear the area"))),
            new InstantCommand(() -> orchestra.play()), Commands.waitSeconds(musicTime),
            new InstantCommand(() -> orchestra.stop()),
            shooterHood.setStateCommand(shooterhood_state.OUT), Commands.waitSeconds(testTime),
            shooterHood.setStateCommand(shooterhood_state.IN)));

    testLayout.add("test Drivetrain translation",
        Commands.sequence(
            new InstantCommand(() -> Elastic.sendNotification(new Elastic.Notification(
                Elastic.NotificationLevel.ERROR, "Robot Enabling", "Clear the area"))),
            new InstantCommand(() -> orchestra.play()), Commands.waitSeconds(musicTime),
            new InstantCommand(() -> orchestra.stop()),
            drivetrain.runOnce(() -> drivetrain.setControl(driveRequest.withVelocityX(0.5))),
            Commands.waitSeconds(drivetrainTestTime),
            drivetrain.runOnce(() -> drivetrain.setControl(new SwerveRequest.Idle()))));

    testLayout.add("test Drivetrain rotation",
        Commands.sequence(
            new InstantCommand(() -> Elastic.sendNotification(new Elastic.Notification(
                Elastic.NotificationLevel.ERROR, "Robot Enabling", "Clear the area"))),
            new InstantCommand(() -> orchestra.play()), Commands.waitSeconds(musicTime),
            new InstantCommand(() -> orchestra.stop()),
            drivetrain.runOnce(() -> drivetrain.setControl(driveRequest.withRotationalRate(1.0))),
            Commands.waitSeconds(drivetrainTestTime),
            drivetrain.runOnce(() -> drivetrain.setControl(new SwerveRequest.Idle()))));
  }
  ...
  private static void updateEntry(String name, TalonFX motor) {
    if (entryMap.get(name + " Connected") != null) {
      ...
      entryMap.get(name + " Powered")
          .setBoolean(motor.getSupplyVoltage().getValueAsDouble() > poweredThreshold);
    }
  }
  ...
  private static void updateEntry(String name, CANcoder encoder) {
    if (entryMap.get(name + " Connected") != null) {
      ...
      entryMap.get(name + " Powered")
          .setBoolean(encoder.getSupplyVoltage().getValueAsDouble() > poweredThreshold);
    }
  }
  ...
  private static void updateEntry(String name, Pigeon2 pigeon) {
    if (entryMap.get(name + " Connected") != null) {
      ...
      entryMap.get(name + " Powered")
          .setBoolean(pigeon.getSupplyVoltage().getValueAsDouble() > poweredThreshold);
    }
  }
}
