...
public class LimelightWrapper extends Limelight {
  ...
  public LimelightWrapper(String limelightName, boolean isLL4) {
    ...
    SubsystemStatusManager.addSubsystem(limelightName,
        () -> getNTTable().getTopic("tv").exists());
    DeviceTempReporter.addDevice(limelightName, () -> Celsius
        .of(getNTTable().getEntry("hw").getDoubleArray(new double[4])[3]));
  }
  ...
  public Matrix<N3, N1> getEstimationStdDevsLimelightMT1(
    ...
    Logger.recordOutput("Robot/Limelights/" + limelightName + "/MT1StdDev",
        estStdDevs.getData());
  ...
  public Matrix<N3, N1> getEstimationStdDevsLimelightMT2(
    ...
    Logger.recordOutput("Robot/Limelights/" + limelightName + "/MT2StdDev",
        estStdDevs.getData());
  ...
  public void updateLocalizationLimelight(CommandSwerveDrivetrain drivetrain) {
    ...
    getSettings().withRobotOrientation(new Orientation3d(
        new Rotation3d(drivetrain.getState().Pose.getRotation()),
        new AngularVelocity3d(
            RadiansPerSecond
                .of(drivetrain.getState().Speeds.omegaRadiansPerSecond),
            RadiansPerSecond.of(0), RadiansPerSecond.of(0))))
        .save();

    // Get MegaTag2 pose

    Optional<PoseEstimate> visionEstimateMt2 =
        Optional.of(estimateMt2).get().getPoseEstimate();

    // If the pose is present
    visionEstimateMt2
        .ifPresent((limelight.networktables.PoseEstimate poseEstimate) -> {
          // Publish average tag count
          totalTags += visionEstimateMt2.get().tagCount;
          roboRioHeartbeat++;
          Logger.recordOutput(
              "Robot/Limelights/" + limelightName + "/AvgTagCount",
          ...
          Logger.recordOutput("Robot/Limelights/" + limelightName + "/TagPoses",
              tagPoses);

          // If we see >0 tags and robot rotates <2 rotations per second
          if (poseEstimate.tagCount > 0 && Math.abs(Units.radiansToRotations(
              drivetrain.getState().Speeds.omegaRadiansPerSecond)) < 2) {
    ...
    if (isLL4) {
      Optional<PoseEstimate> visionEstimateMt1 =
          Optional.of(estimateMt1).get().getPoseEstimate();

      // If the pose is present
      visionEstimateMt1
          .ifPresent((limelight.networktables.PoseEstimate poseEstimate) -> {
            // And we see >1 tags and robot rotates <2 rotations per second
            if (poseEstimate.tagCount > 1 && Math.abs(Units.radiansToRotations(
                drivetrain.getState().Speeds.omegaRadiansPerSecond)) < 2) {
              // Add it to the pose estimator.
              drivetrain.addVisionMeasurement(poseEstimate.pose.toPose2d(),
