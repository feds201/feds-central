...
public class RobotContainer extends ControllerBindings {
  ...
  private final CommandSwerveDrivetrain drivetrain =
      DrivetrainConstants.createDrivetrain();
  ...
  private final LimelightWrapper llMain =
      new LimelightWrapper("limelight-two", true);
  private final LimelightWrapper llBackup =
      new LimelightWrapper("limelight-five", true);

  private static java.io.File usb = RobotMap.PitConstants.usb;


  private final CommandXboxController controller = new CommandXboxController(0);
  private final CommandXboxController operaterController =
  ...
  public RobotContainer() {
    ...
    try {
      ...
      registBallTrackingAuto("Dev-FD-MidIntakeToLeftBump",
      ...
      registBallTrackingAuto("Dev-FD-RightSneakDoublepass",
          ...
          List.of("Internal-FD-RightSneakDoublepass-Part2",
  ...
  private void registBallTrackingAuto(String autoName, String part1Name,
    ...
    for (String part : postTrackingParts) {
      Pose2d returnPose = PathPlannerAuto.getPathGroupFromAutoFile(part).get(0)
          .getStartingHolonomicPose().get();
      autoCommand =
          autoCommand.andThen(new BallTracking(drivetrain).withTimeout(3.0))
              .andThen(AutoBuilder.pathfindToPose(returnPose,
   ...
   */
  public synchronized void startAutoSweep(double min, double max, double step,
      double hoodDeg, int holdMs) {
  ...
  public synchronized void startAutoSweepFromDiagnostic(int holdMs) {
    ...
    shooterWheels.setState(
        frc.robot.subsystems.shooter.ShooterWheels.shooter_state.TEST);
    shooterHood.setState(
        frc.robot.subsystems.shooter.ShooterHood.shooterhood_state.TEST);
    ...
    autoSweeper.startDynamic(
        ...
        () -> {
          shooterWheels.setState(
              frc.robot.subsystems.shooter.ShooterWheels.shooter_state.IDLE);
          shooterHood.setState(
              frc.robot.subsystems.shooter.ShooterHood.shooterhood_state.IN);
        }, holdMs);
  }
  ...
  public void initSimulation() {
    simManager = new RebuiltSimManager(drivetrain, intakeSubsystem,
        feederSubsystem, shooterWheels, shooterHood, spinDexer, ledsSubsystem);
  ...
  private void configureRootTests() {
    ...
    rtumanager.setSafetyCheck(() -> {
      ...
      boolean triggersOk = controller.getLeftTriggerAxis() >= 0.5
          && controller.getRightTriggerAxis() >= 0.5;

      boolean xyOk =
          controller.getHID().getXButton() && controller.getHID().getYButton();
  ...
  public void registerNamedCommands() {
    ...
    NamedCommands.registerCommand("Start Shooter Spin",
        shooterWheels.setStateCommand(shooter_state.SHOOTING).alongWith(
    ...
    NamedCommands.registerCommand("Passing",
        shooterWheels.setStateCommand(shooter_state.PASSING)
            ...
            .alongWith(spinDexer.setStateCommand(spindexer_state.RUN))
            .alongWith(shooterHood.setStateCommand(shooterhood_state.PASSING)));
    NamedCommands.registerCommand("Auto Hub Drive",
        new HubDriveAUTO(drivetrain));
  }


  private final ShuffleboardLayout llLayout = Shuffleboard.getTab("Pit Testing")
      .getLayout("Limelight Health", BuiltInLayouts.kList).withSize(2, 1)
      .withPosition(4, 5);
  private GenericEntry llBackupConnect =
      llLayout.add("LL Backup Connected", false).getEntry();
  private GenericEntry llMainConnect =
      llLayout.add("LL Main Connected", false).getEntry();
  ...
  public void usbStorage() {
    ...
    boolean storageOk =
        mounted && freeBytes >= RobotMap.PitConstants.STORAGE_ACCEPTABLE_BYTES;
    String label = !mounted ? "NO DRIVE"
        : String.format("%.1f GB free / %.1f GB total", freeBytes / 1e9,
            totalBytes / 1e9);
