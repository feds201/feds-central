...
public class LedsSim {
  ...
  /**
   * Constructor for LedsSim. Handles simulation-specific configuration if running in a simulation
   * environment. * @param leds The ConnectorXAnimate instance to be configured.
   */
  ...
  private LumynDeviceConfig buildConfig() {
    return new ConfigBuilder().forTeam("201")

        .addChannel(1, "port1", 60).addStripZone("ZONE_LEFT", 60, false).endChannel()
        .addChannel(2, "port2", 60).addStripZone("ZONE_RIGHT", 60, false).endChannel()

        // Groups
        .addGroup("GR_ALL").addZone("ZONE_LEFT").addZone("ZONE_RIGHT").endGroup().build();
  }
}
