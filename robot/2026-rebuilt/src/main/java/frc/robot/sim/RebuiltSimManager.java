...
public class RebuiltSimManager {
  ...
  // TODO(ai-idea)?: move these flywheel-ball physics defaults into sim-core? applies to any
  // flywheel
  // shooter, not just 2026.
  /**
   * How much to reduce force put into ball translation based on slipping of shooter wheels AND
   * energy put into backspinning the ball
   */
   ...
   *
   * <p>
   * Sets up MapleSim for drivetrain physics and ODE4J for game piece physics, wires motor
   * controller adapters, spawns starting fuel, and configures intake/shooter/scoring systems.
   *
  ...
  public RebuiltSimManager(CommandSwerveDrivetrain drivetrain, Intake intakeSubsystem,
      FeederSubsystem feeder, ShooterWheels shooterWheels, ShooterHood shooterHood,
    ...
    Translation2d[] modulePositions =
        new Translation2d[] {new Translation2d(MODULE_OFFSET_M, MODULE_OFFSET_M), // FL
    ...
    DriveTrainSimulationConfig driveSimConfig =
        DriveTrainSimulationConfig.Default().withRobotMass(Kilograms.of(ROBOT_MASS_KG))
            ...
            .withCustomModuleTranslations(modulePositions).withSwerveModule(moduleSimConfig);
    ...
    for (int i = 0; i < 4; i++) {
      moduleSimulations[i]
          .useDriveMotorController(new TalonFXMotorControllerSim(drivetrain.getDriveMotor(i)));
      moduleSimulations[i].useSteerMotorController(new TalonFXMotorControllerWithRemoteCanCoderSim(
    ...
    ChassisConfig chassisConfig = new ChassisConfig.Builder().withModulePositions(modulePositions)
        .withRobotMass(ROBOT_MASS_KG).withRobotMOI(ROBOT_MOI)
        .withBumperSize(BUMPER_LENGTH_M, BUMPER_WIDTH_M, BUMPER_HEIGHT_M).build();
    ...
    // - Shooting gate: velocity-based (feeder forward-only check handles washing machine reverse
    // phase)
    ...
    shooterSim = new ShooterSim(gamePieceManager, RebuiltGamePieces.FUEL, () -> chassis.getPose2d(),
        // Hood convention: higher hood physical angle = FLATTER ball launch (hood deflector raises
        // out of
    ...
    Transform3d llMainMount = new Transform3d(new Translation3d(0.2329, 0.2332, 0.5173),
        new Rotation3d(0, Math.toRadians(-24.31), Math.toRadians(-20)));
    Transform3d llBackupMount = new Transform3d(new Translation3d(0.31, -0.284, 0.193),
        new Rotation3d(0, Math.toRadians(-25), Math.toRadians(20)));
    LimelightSim llMainCam =
        new LimelightSim(new CameraConfig("limelight-two", LimelightType.LL4, llMainMount));
    LimelightSim llBackupCam =
        new LimelightSim(new CameraConfig("limelight-five", LimelightType.LL4, llBackupMount));
    LimelightSim fuelCam = new LimelightSim(
        new CameraConfig(FUEL_LL_NAME, LimelightType.LL3, FUEL_LL_MOUNT), FUEL_LL_NEAR, FUEL_LL_FAR,
        RebuiltGamePieces.FUEL.getRadius(), FUEL_LL_FPS, BallTracking::isActive);
  ...
  public void periodic() {
    ...
    gamePieceManager.updateProximity(chassis.getPose2d().getTranslation(), PROXIMITY_WAKE_RADIUS,
        PROXIMITY_SLEEP_RADIUS);
    ...
    visionSimManager.updateGamePieces(pose,
        () -> gamePieceManager.getActivePieces().stream().map(GamePiece::getPosition3d).toList());
    ...
    double batteryVoltage =
        BatterySimUtil.updateBatteryVoltage(7.0, 12.5, shooterMotorSim.getCurrentDrawAmps() * 4, // leader
                                                                                                 ...
                                                                                                 // flywheel
                                                                                                 // load
            hoodArmSim.getCurrentDrawAmps(), feederSubsystem.getCurrent().in(Amps),
    ...
    Logger.recordOutput("Sim/Debug/ShootingGateOpen",
        shooterMotorSim.getAngularVelocityRPS() > SHOOTER_VELOCITY_THRESHOLD_RPS && feederSubsystem
            .getFeederVelocity().in(RotationsPerSecond) > FEEDER_VELOCITY_THRESHOLD_RPS);
  ...
  // TODO(ai-idea)?: split publishTelemetry into a RebuiltComponentPoseTelemetry class and invoke
  // via
  // hook?
  private void publishTelemetry() {
    ...
    visionSimManager.getDirectionLines(robotPose).forEach(
        (name, line) -> Logger.recordOutput("Sim/Limelights/" + name + "/DirectionLine", line));
    ...
    Logger.recordOutput("Sim/ComponentPoses", new Pose3d[] {new Pose3d(), // 0: Intake - Stationary
        ...
        new Pose3d(new Translation3d(0.002, 0.0635, 0.184), new Rotation3d(0, 0, spindexerAngle)), // 4:
                                                                                                   // Spindexer
                                                                                                   // -
                                                                                                   // Omni
                                                                                                   // 1
        new Pose3d(new Translation3d(0.002, 0.0635, 0.318), new Rotation3d(0, 0, spindexerAngle)), // 5:
                                                                                                   // Spindexer
                                                                                                   // -
                                                                                                   // Star
                                                                                                   // 1
        new Pose3d(new Translation3d(0.002, -0.0635, 0.184), new Rotation3d(0, 0, -spindexerAngle)), // 6:
                                                                                                     // Spindexer
                                                                                                     // -
                                                                                                     // Omni
                                                                                                     // 2
        new Pose3d(new Translation3d(0.002, -0.0635, 0.318), new Rotation3d(0, 0, -spindexerAngle)), // 7:
                                                                                                     // Spindexer
                                                                                                     // -
                                                                                                     // Star
                                                                                                     // 2
        new Pose3d(), // 8: Feeder - Stationary
        new Pose3d(new Translation3d(0.118, 0, 0.280), new Rotation3d(0, -feederAngle, 0)), // 9:
                                                                                            // Feeder
                                                                                            // -
                                                                                            // Lower
                                                                                            // Front
        new Pose3d(new Translation3d(0.316, 0, 0.257), new Rotation3d(0, feederAngle, 0)), // 10:
                                                                                           // Feeder
                                                                                           // -
                                                                                           // Lower
                                                                                           // Back
        new Pose3d(new Translation3d(0.075, 0, 0.380), new Rotation3d(0, -feederAngle, 0)), // 11:
                                                                                            // Feeder
                                                                                            // - Top
                                                                                            // Front
        new Pose3d(new Translation3d(0.270, 0, 0.360), new Rotation3d(0, feederAngle, 0)), // 12:
                                                                                           // Feeder
                                                                                           // - Top
                                                                                           // Back
        ...
        new Pose3d(new Translation3d(0.286, 0, 0.558), new Rotation3d(0, shooterAngle, 0)), // 15:
                                                                                            // Shooter
                                                                                            // -
                                                                                            // Wheels
    });
  ...
  //
  // MapleSim ---(position, velocity)---> Adapter ---(setRawRotorPosition, etc.)---> TalonFX
  // SimState
   ...
   *
   * <p>
   * Battery voltage is sourced from {@link SimulatedBattery} so brownout effects propagate through
   ...
   * position feedback. MapleSim provides the mechanism angle (steering position), which this
   * adapter writes to both the TalonFX rotor state and the CANcoder's raw position so that both
   * sensors agree.
   */
