 ...
 *
 * MapleSim owns the entire drivetrain (motor physics, tire model, chassis dynamics). The ODE4J
 * chassis is a kinematic follower that exists only for game piece collisions.
 *
 * Architecture: - MapleSim: steps drivetrain, computes pose and velocities, writes encoder state -
...
public class RebuiltSimManager {
  ...
  public static final Pose2d STARTING_POSE =
      new Pose2d(4.434, 7.532, Rotation2d.fromDegrees(0.));
  ...
  /**
   * These 2 values combined MUST ensure we clear the chassis otherwise the physics engine will have
   * balls colliding w/ the robot immediately upon shooting
   */
  private static final double LAUNCH_HEIGHT = 0.55;
  private static final double MUZZLE_FORWARD_OFFSET =
      SHOOTER_HOOD_FORWARD_X - RebuiltGamePieces.FUEL.getRadius();
  ...
  /**
   * Shooter wheel radius (m) — used to convert flywheel RPS to launch speed (m/s). TODO update
   * placeholder
   */
  private static final double SHOOTER_WHEEL_RADIUS_M = 0.05;

  // TODO(ai-idea)?: move these flywheel-ball physics defaults into sim-core? applies to any flywheel
  // shooter, not just 2026.
  /**
   * How much to reduce force put into ball translation based on slipping of shooter wheels AND energy
   * put into backspinning the ball
   */
  ...
  private static final double HOOD_GEAR_RATIO =
      30.0 * 2 * Math.PI / Math.toRadians(67.4 - 35.5);
   ...
   */
  private static final double HOOD_MOI_KGM2 =
      SingleJointedArmSim.estimateMOI(0.23, 3.0); // 23cm arm, 3kg
  ...
  private static final double HOOD_MIN_ANGLE_DEG =
      RobotMap.ShooterConstants.HOOD_MIN_ANGLE_DEG;

  /** Hood maximum angle (degrees from horizontal) — motor position = HOOD_FORWARD_SOFT_LIMIT_ROT. */
  private static final double HOOD_MAX_ANGLE_DEG =
      RobotMap.ShooterConstants.HOOD_MAX_ANGLE_DEG;
  ...
  /**
   * Feeder velocity threshold (RPS, forward-only) — negative velocity (reverse phase) stops shooting.
  ...
  /**
   * Intake deploy MOI (kg·m²). Sim tuning knob, not measured from CAD yet. May need retuning now that
   * the gear ratio is physical.
   */
  ...
  private static final Transform3d FUEL_LL_MOUNT =
      new Transform3d(new Translation3d(-0.50, 0, 0.325),
   ...
   *
   * <p>
   * Sets up MapleSim for drivetrain physics and ODE4J for game piece physics, wires motor controller
   * adapters, spawns starting fuel, and configures intake/shooter/scoring systems.
   *
  ...
  public RebuiltSimManager(CommandSwerveDrivetrain drivetrain,
      ...
      ShooterWheels shooterWheels, ShooterHood shooterHood, Spindexer spindexer,
    ...
    SwerveModuleSimulationConfig moduleSimConfig =
        new SwerveModuleSimulationConfig(DCMotor.getFalcon500(1),
    ...
    DriveTrainSimulationConfig driveSimConfig = DriveTrainSimulationConfig
        .Default().withRobotMass(Kilograms.of(ROBOT_MASS_KG))
    ...
    ChassisConfig chassisConfig =
        new ChassisConfig.Builder().withModulePositions(modulePositions)
            .withRobotMass(ROBOT_MASS_KG).withRobotMOI(ROBOT_MOI)
    ...
    intakeZone = new IntakeZone(INTAKE_X_MIN, INTAKE_X_MAX, INTAKE_Y_MIN,
        INTAKE_Y_MAX, INTAKE_Z_MAX,
        () -> intakeSubsystem
            .getSimDeployMotorPositionRotations() > IntakeSubsystem.extendedRotations
                - 0.5
            && intakeSubsystem
    ...
    shooterMotorSim =
        new TalonFXMotorSim(shooterWheels.getShooterLeaderMotorSimState(),
    ...
    hoodArmSim = new TalonFXArmSim(shooterHood.getHoodMotorSimState(),
        new SingleJointedArmSim(DCMotor.getKrakenX60(1), HOOD_GEAR_RATIO,
            HOOD_MOI_KGM2, HOOD_ARM_LENGTH_M,
            Math.toRadians(HOOD_MIN_ANGLE_DEG),
            Math.toRadians(HOOD_MAX_ANGLE_DEG), true,
            Math.toRadians(HOOD_MIN_ANGLE_DEG)),
    ...
    feederMotorSim = new TalonFXMotorSim(feeder.getFeederMotorSimState(),
        new DCMotorSim(LinearSystemId.createDCMotorSystem(
            DCMotor.getKrakenX60(1), FEEDER_MOI, FEEDER_GEAR_RATIO),
            DCMotor.getKrakenX60(1)),
        ...
        new TalonFXMotorSim(spindexer.getSpindexerMotorSimState(),
            new DCMotorSim(
                LinearSystemId.createDCMotorSystem(DCMotor.getKrakenX60(1),
                    SPINDEXER_MOI, SPINDEXER_GEAR_RATIO),
                DCMotor.getKrakenX60(1)),
            SPINDEXER_GEAR_RATIO, true);

    intakeDeployMotorSim =
        new TalonFXMotorSim(intakeSubsystem.getDeployMotorSimState(),
            new DCMotorSim(
                LinearSystemId.createDCMotorSystem(DCMotor.getKrakenX60(1),
                    INTAKE_DEPLOY_MOI, INTAKE_DEPLOY_GEAR_RATIO),
                DCMotor.getKrakenX60(1)),
            INTAKE_DEPLOY_GEAR_RATIO, false);

    // Two Krakens in parallel drive the rollers (one each side).
    intakeRollerMotorSim =
        new TalonFXMotorSim(intakeSubsystem.getRollerMotorSimState(),
            new DCMotorSim(
                LinearSystemId.createDCMotorSystem(DCMotor.getKrakenX60(2),
                    INTAKE_ROLLER_MOI, INTAKE_ROLLER_GEAR_RATIO),
    ...
    // TODO(ai-idea)?: pull this (flywheelRadPerSec, wheelRadius, ballRadius, slip, fraction) into a
    // FlywheelBallPhysics helper so callers don't wire the formula by hand?
    shooterSim = new ShooterSim(gamePieceManager, RebuiltGamePieces.FUEL,
        () -> chassis.getPose2d(),
        ...
        () -> (Math.PI / 2.0) - hoodArmSim.getAngleRads()
            + Math.toRadians(BALL_ANGLE_OFFSET_DEG),
        () -> shooterMotorSim.getAngularVelocityRadPerSec()
            * SHOOTER_WHEEL_RADIUS_M * SHOOTER_EFFICIENCY_FACTOR,
        () -> shooterMotorSim
            .getAngularVelocityRPS() > SHOOTER_VELOCITY_THRESHOLD_RPS
            && feederMotorSim
                .getAngularVelocityRPS() > FEEDER_VELOCITY_THRESHOLD_RPS,
        () -> chassis.getBody().getLinearVel().get0(),
        () -> chassis.getBody().getLinearVel().get1(), LAUNCH_HEIGHT,
        MUZZLE_FORWARD_OFFSET, BARREL_LATERAL_OFFSET, SHOTS_PER_SECOND);

    // --- Scoring & Telemetry ---
    Logger.recordOutput("Sim/State", "Loading scoring");
    scoringTracker = new ScoringTracker();
    groundClearance =
        new GroundClearance(chassis.getBody(), chassisConfig.getBumperHeight());

    // --- Vision sim (writes true pose to NT in Limelight format) ---
    Logger.recordOutput("Sim/State", "Loading vision");

    Transform3d llMainMount =
        new Transform3d(new Translation3d(0.2329, 0.2332, 0.5173),
            new Rotation3d(0, Math.toRadians(-24.31), Math.toRadians(-20)));
    Transform3d llBackupMount =
        new Transform3d(new Translation3d(0.31, -0.284, 0.193),
    ...
    LimelightSim fuelCam = new LimelightSim(
        ...
        FUEL_LL_NEAR, FUEL_LL_FAR, RebuiltGamePieces.FUEL.getRadius(),
        FUEL_LL_FPS, BallTracking::isActive);
   ...
   *
   * <p>
   * Sequence: MapleSim steps drivetrain → read pose/speeds → hard-sync planar state into ODE4J →
   * proximity activation → intake check → physics step → gyro sync → piece state update → scoring
   * check → shooter update → telemetry.
   */
  public void periodic() {
    // TODO(ai-idea)?: extract this periodic() scaffold into a sim-core SimOrchestrator base class?
    // might be reusable year over year.
    ...
    double worldVx = robotSpeeds.vxMetersPerSecond * cos
        - robotSpeeds.vyMetersPerSecond * sin;
    double worldVy = robotSpeeds.vxMetersPerSecond * sin
        + robotSpeeds.vyMetersPerSecond * cos;
    chassis.setPlanarState(pose, worldVx, worldVy,
        robotSpeeds.omegaRadiansPerSecond);

    // Proximity activation — only wake balls near the robot
    gamePieceManager.updateProximity(chassis.getPose2d().getTranslation(),
    ...
    pigeonSimState
        .setSupplyVoltage(SimulatedBattery.getBatteryVoltage().in(Volts));
    pigeonSimState.setRawYaw(Math.toDegrees(pose.getRotation().getRadians()));
    pigeonSimState
        .setAngularVelocityZ(Math.toDegrees(robotSpeeds.omegaRadiansPerSecond));
    ...
    visionSimManager.updateGamePieces(pose, () -> gamePieceManager
        .getActivePieces().stream().map(GamePiece::getPosition3d).toList());
    ...
    Logger.recordOutput("Sim/Debug/HoodAngleDeg",
        Math.toDegrees(hoodArmSim.getAngleRads()));
    ...
    Logger.recordOutput("Sim/Debug/IntakeDeployExtendedPct",
        Math.min(100.0,
            Math.max(0.0, intakeSubsystem.getSimDeployMotorPositionRotations()
    ...
    Logger.recordOutput("Sim/Debug/FuelHeld",
        (double) gamePieceManager.getHeldCount());
  ...
  // TODO(ai-idea)?: split publishTelemetry into a RebuiltComponentPoseTelemetry class and invoke via
  // hook?
  private void publishTelemetry() {
    ...
    Translation2d virtualGoal =
        ShootOnTheMove.calculateVirtualGoal(lastSimPose, lastSimSpeeds);
    Logger.recordOutput("Sim/VirtualGoal", new Pose3d(virtualGoal.getX(),
        virtualGoal.getY(), FieldConstants.Hub.innerHeight, new Rotation3d()));

    // Limelight direction lines (sim-only visualization)
    visionSimManager.getDirectionLines(robotPose).forEach((name, line) -> Logger
        .recordOutput("Sim/Limelights/" + name + "/DirectionLine", line));

    Logger.recordOutput("Sim/RobotGroundClearance",
        groundClearance.getClearance());
    ...
    gamePieceManager
        .publishPoses((key, poses) -> Logger.recordOutput(key, poses));
    ...
    double deployFraction = Math.min(1.0,
        Math.max(0.0, intakeSubsystem.getSimDeployMotorPositionRotations()
            / IntakeSubsystem.extendedRotations));
    Translation3d intakeExtension = new Translation3d(
        INTAKE_EXTEND_X * deployFraction, 0, INTAKE_EXTEND_Z * deployFraction);

    double rollerAngle =
        -(intakeSubsystem.getSimRollerMotorPositionRotations() * 2 * Math.PI)
            % (2 * Math.PI);
    double spindexerAngle =
        (spindexerMotorSim.getAngularPositionRotations() * 2 * Math.PI)
            % (2 * Math.PI);
    double feederAngle =
        (feederMotorSim.getAngularPositionRotations() * 2 * Math.PI)
            % (2 * Math.PI);
    double shooterAngle =
        (shooterMotorSim.getAngularPositionRotations() * 2 * Math.PI)
            % (2 * Math.PI);
    ...
    Logger.recordOutput("Sim/ComponentPoses", new Pose3d[] {new Pose3d(), // 0: Intake - Stationary
        ...
        new Pose3d(intakeExtension.plus(new Translation3d(-0.2812, 0, 0.2627)),
            new Rotation3d(0, rollerAngle, 0)), // 2: Intake - Rollers
        new Pose3d(), // 3: Spindexer - Assembly
        new Pose3d(new Translation3d(0.002, 0.0635, 0.184),
            new Rotation3d(0, 0, spindexerAngle)), // 4: Spindexer - Omni 1
        new Pose3d(new Translation3d(0.002, 0.0635, 0.318),
            new Rotation3d(0, 0, spindexerAngle)), // 5: Spindexer - Star 1
        new Pose3d(new Translation3d(0.002, -0.0635, 0.184),
            new Rotation3d(0, 0, -spindexerAngle)), // 6: Spindexer - Omni 2
        new Pose3d(new Translation3d(0.002, -0.0635, 0.318),
            new Rotation3d(0, 0, -spindexerAngle)), // 7: Spindexer - Star 2
        new Pose3d(), // 8: Feeder - Stationary
        new Pose3d(new Translation3d(0.118, 0, 0.280),
            new Rotation3d(0, -feederAngle, 0)), // 9: Feeder - Lower Front
        new Pose3d(new Translation3d(0.316, 0, 0.257),
            new Rotation3d(0, feederAngle, 0)), // 10: Feeder - Lower Back
        new Pose3d(new Translation3d(0.075, 0, 0.380),
            new Rotation3d(0, -feederAngle, 0)), // 11: Feeder - Top Front
        new Pose3d(new Translation3d(0.270, 0, 0.360),
            new Rotation3d(0, feederAngle, 0)), // 12: Feeder - Top Back
        new Pose3d(), // 13: Shooter - Stationary Base
        new Pose3d(new Translation3d(SHOOTER_HOOD_FORWARD_X, 0.0, 0.500),
            new Rotation3d(0, hoodAngle, 0)), // 14: Shooter - Hood
        new Pose3d(new Translation3d(0.286, 0, 0.558),
            new Rotation3d(0, shooterAngle, 0)), // 15: Shooter - Wheels
    });
  }
   ...
   *
   * <p>
   * Each tick, MapleSim calls {@link #updateControlSignal} with the mechanism state it computed
   * (position and velocity). This adapter writes those values into the TalonFX's sim state so that
   * robot code sees realistic encoder readings, then returns the motor's commanded voltage back to
   * MapleSim for force computation.
   *
   * <p>
   * Battery voltage is sourced from {@link SimulatedBattery} so brownout effects propagate through to
   * the motor controller.
   */
  // TODO(ai-idea)?: move these CTRE/MapleSim adapter classes to sim-core? no 2026-specific content.
  private static class TalonFXMotorControllerSim
   ...
   *
   * <p>
   * Used for swerve steer modules where the TalonFX uses a fused/remote CANcoder for absolute
   * position feedback. MapleSim provides the mechanism angle (steering position), which this adapter
   * writes to both the TalonFX rotor state and the CANcoder's raw position so that both sensors
   * agree.
   */
  private static class TalonFXMotorControllerWithRemoteCanCoderSim
    ...
    TalonFXMotorControllerWithRemoteCanCoderSim(TalonFX talonFX,
        CANcoder cancoder) {
      super(talonFX);
