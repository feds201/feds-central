...
public class PathfindToPose extends Command {
  ...
  // tune these values
  private final PIDController xPID = new PIDController(RobotMap.PathfindConstants.xP,
      RobotMap.PathfindConstants.xI, RobotMap.PathfindConstants.xD);
  private final PIDController yPID = new PIDController(RobotMap.PathfindConstants.yP,
      RobotMap.PathfindConstants.yI, RobotMap.PathfindConstants.yD);
  private final PIDController rotPID = new PIDController(RobotMap.PathfindConstants.rotP,
      RobotMap.PathfindConstants.rotI, RobotMap.PathfindConstants.rotD);
  ...
  public PathfindToPose(CommandSwerveDrivetrain dt, Pose2d targetPose) {
    ...
    driveRequest = new SwerveRequest.FieldCentric().withDeadband(MAX_SPEED * .07)
  ...
  public void execute() {
    ...
    double rotationOutput = rotPID.calculate(currentPose.getRotation().getDegrees(), // current
                                                                                     // rotation
        targetPose.getRotation().getDegrees()); // target rotation
    double xTransOutput = xPID.calculate(currentPose.getX(), targetPose.getX());
    double yTransOutput = yPID.calculate(currentPose.getY(), targetPose.getY());

    double maxAngularRateDegreesPerSecond = RotationsPerSecond.of(2).in(DegreesPerSecond);
    rotationOutput = MathUtil.clamp(rotationOutput, -maxAngularRateDegreesPerSecond,
    ...
    dt.setControl(driveRequest.withVelocityX(MetersPerSecond.of(xTransOutput))
