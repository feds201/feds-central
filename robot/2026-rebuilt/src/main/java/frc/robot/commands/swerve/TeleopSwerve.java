...
public class TeleopSwerve extends Command {
  ...
  @Override
  public void execute() {
    SmartDashboard.putNumber("Dist to hub",
        dt.getState().Pose.getTranslation().getDistance(ShooterConstants.hubCenter));
    ...
    switch (mode) {
      case FALCONDRIVE:
        ...
        if (Math.hypot(rightX, rightY) > 0.7) { // Deadband check
          ...
          AngularVelocity joystickAngularVelocity = angleDifference.div(deltaTime); // degrees per
                                                                                    // second
        ...
        dt.setControl(
            falconDrive.withVelocityX(-controller.getLeftY() * MAX_SPEED * speedMultiplier)
      ...
      case HUBDRIVE:
        ...
        // 2 Get theta to virtual goal (will be equivalent to angle to hub when stationary, but
        // leads the
        // target when moving)
        Double angleToGoal = ShootOnTheMove
            .calculateRobotHeading(dt.getState().Pose, dt.getState().Speeds).getDegrees();
        ...
        dt.setControl(driveNormal.withVelocityX(-controller.getLeftY() * 2 * speedMultiplier)
