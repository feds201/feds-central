...
public class BallTracking extends Command {
  ...
  private final double velocityCapX = 3.0; // velocity that robot will move when fuel is detected,
                                           // max velocity when detected
  private final double rotationalRate = 1.25; // amount of rotation the robot preforms when
                                              // searching for fuel, in radians per second
  private final double tyOffset = 23.0; // used to scale foward velocity based on ty
  private final double tyCutOff = -15.0; // limit to ty, if ty is less then num robot will stop and
                                         // look for ball
  ...
  public boolean withinBlueAlliance() {
    Translation2d blueAllianceTranslation2d =
        new Pose2d(7.7, 7.5, new Rotation2d(0)).getTranslation();
    Rectangle2d blueRectangle2d =
        new Rectangle2d(blueAllianceTranslation2d, new Translation2d(0.72, 0.56));
  ...
  public boolean withinRedAlliance() {
    Translation2d redAllianceTranslation2d =
        new Pose2d(16.0, 7.5, new Rotation2d(0)).getTranslation();
    Rectangle2d redRectangle2d =
        new Rectangle2d(redAllianceTranslation2d, new Translation2d(9.0, 0.56));
    Pose2d robotPose = (dt.getState().Pose);
  ...
  public boolean withinTransition() {
    Translation2d TransitionTranslation2d =
        new Pose2d(8.0, 7.55, new Rotation2d(0)).getTranslation();
    Rectangle2d TransitionRectangle2d =
        new Rectangle2d(TransitionTranslation2d, new Translation2d(8.0, 0.5));
    Pose2d robotPose = (dt.getState().Pose);
  ...
  public void execute() {
    ...
    if (true) {
      ...
      if (hasTarget) {
        ...
        if (ty > tyCutOff) {
          dt.setControl(driveNormal.withVelocityX(forwardVelocity).withVelocityY(0)
        ...
        dt.setControl(
            driveNormal.withVelocityX(0).withVelocityY(0).withRotationalRate(rotationalRate));
    ...
    if (false) {
      if (withinTransition()) {
        dt.setControl(driveNormal.withVelocityX(velocityCapX).withVelocityY(0)
            .withRotationalRate(rotationalRate));
      }
  ...
  private void stopMovement() {
    dt.setControl(driveNormal.withVelocityX(0).withVelocityY(0).withRotationalRate(0));
    Logger.recordOutput("Robot/Intake/BallFinder/CommandedVx", 0.0);
