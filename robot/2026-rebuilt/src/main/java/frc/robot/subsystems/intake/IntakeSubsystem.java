...
public class IntakeSubsystem extends SubsystemBase {
  ...
  private final MotionMagicVoltage positionOut =
      new MotionMagicVoltage(Rotations.of(0));
  ...
  public enum IntakeState {
    ...
    AGITATE_IN, // Inwards portion of the agitate state-loop, intake will toggle between the agitates on a timer
                // when set to one of these states
    AGITATE_OUT, // Agitation causes the intake to move outwards, then inwards back to default in order to agitate
                 // the fuel in the full hopper

    // State loop 2: Close Agitation
    CLOSE_AGITATION_IN, // Inwards portion of the close agitation state-loop, intake will toggle between the close agitates
                        // on a timer when set to one of these states
    CLOSE_AGITATION_OUT, // Close agitation causes the intake to move outwards, then inwards about halfway to extended in
                         // order to agitate the close half of the hopper

    FAR_AGITATION_IN, // Inwards portion of the far agitation state-loop, intake will toggle between the far agitates on a
    ...
    DITHERIN_AGITATION, // Inwards portion of dithering state-loop, intake will toggle between the dithers on a timer when
                        // set to one of these states
    DITHEROUT_AGITATION, // Dithering causes the intake to move inwards, then outwards half as much in order to slowly bring
   ...
   */
  public Command agitateCommand(int pulses, double extendSeconds,
  ...
  /**
   * Pulse the intake (extend -> run roller -> retract -> pause) repeatedly until the Command is
   * canceled. Use this with button.whileTrue(...). This variant uses time durations for each phase.
   *
   ...
   */
  public Command agitateWhileHeldTime(double extendSeconds,
      double retractSeconds) {
    return new AgitateWhileHeldTimeCommand(this, extendSeconds, retractSeconds);
  }

  /**
   * Pulse the intake repeatedly until the Command is canceled. Each extend phase runs the roller for
   * a target number of rotations before retracting.
   *
   * @param rotationsPerPulse roller rotations to run during each extend phase
   */
  /*
   * public Command agitateWhileHeldRotations(double rotationsPerPulse) { return new
   * AgitateWhileHeldRotationsCommand(this, rotationsPerPulse); }
   *
   * /** Run the roller motor until it has turned the requested number of rotations. This Command
   * requires the intake subsystem while running.
   */
  public Command runRollerForRotations(double rotations) {
    ...
    return Commands.sequence(
        ...
        Commands.runOnce(() -> start[0] =
            rollerMotor.getPosition().getValue().in(Units.Rotations)),
        // start roller
        Commands.runOnce(() -> rollerMotor.set(ROLLER_OUTPUT)),
        // wait until requested rotations completed
        new WaitUntilCommand(() -> Math
            .abs(rollerMotor.getPosition().getValue().in(Units.Rotations)
                - start[0]) >= Math.abs(rotations)),
        // stop roller
   ...
   * automatically from the current position. This is useful for oscillating between fixed bounds like
   * 0 and 5 rotations.
   */
    ...
    return Commands.defer(() -> {
      ...
      return Commands.sequence(
          ...
          new WaitUntilCommand(() -> {
            ...
            return direction > 0 ? position >= targetRotations
  ...
  public IntakeSubsystem() {
    ...
    rollerMotorFollower =
        new TalonFX(RobotMap.IntakeSubsystemConstants.kRollerMotorFollowerID);
    rollerMotorFollower.setControl(
        new Follower(rollerMotor.getDeviceID(), MotorAlignmentValue.Opposed));
    limit_switch =
        new DigitalInput(RobotMap.IntakeSubsystemConstants.klimit_switchID);
    ...
    sysID =
        new SysIdRoutine(new SysIdRoutine.Config(),
            new SysIdRoutine.Mechanism(
                (voltage) -> motor
                    .setControl(new VoltageOut(0).withOutput(voltage)),
                (log) -> {
                  log.motor("motor1")
  ...
  public void periodic() {
    ...
    Logger.recordOutput("Robot/Intake/Extended",
        currentState != IntakeState.DEFAULT);
    Logger.recordOutput("Robot/Intake/LimitSwitchPressed", !limit_switch.get());
    Logger.recordOutput("Robot/Intake/ExtensionPct",
        Math.round(100.0 * motor.getPosition().getValue().in(Units.Rotations)
            / extendedRotations));
    ...
    Logger.recordOutput("Robot/Intake/AppliedVolts",
        motor.getMotorVoltage().getValueAsDouble());
    Logger.recordOutput("Robot/Intake/StatorAmps",
        motor.getStatorCurrent().getValueAsDouble());

    Logger.recordOutput("Robot/IntakeRoller/State",
        currentRollerState.toString());
    Logger.recordOutput("Robot/IntakeRoller/VelocityRPS",
        rollerMotor.getVelocity().getValueAsDouble());
    Logger.recordOutput("Robot/IntakeRoller/AppliedVolts",
        rollerMotor.getMotorVoltage().getValueAsDouble());
    Logger.recordOutput("Robot/IntakeRoller/StatorAmps",
  ...
  /**
   * Returns the TalonFX sim state for the deployment motor. Used by RebuiltSimManager to drive the
   * deploy DCMotorSim. Sim use only.
   */
  ...
  /**
   * Returns the TalonFX sim state for the roller motor. Used by RebuiltSimManager to drive the roller
   * DCMotorSim. Sim use only.
   */
  ...
  /**
   * Returns the deployment motor rotor position in rotations. Used by RebuiltSimManager to determine
   * whether the intake is extended far enough to activate the intake zone (compared against
   * IntakeSubsystem.extendedRotations). Sim use only.
   */
  ...
  /**
   * Returns the roller motor velocity in rotations per second. Used by RebuiltSimManager to determine
   * whether the roller is spinning fast enough to count as actively intaking (compared against
   * INTAKE_ROLLER_VELOCITY_THRESHOLD_RPS). Sim use only.
   */
  ...
  /**
   * Returns the roller motor rotor position in rotations. Used by RebuiltSimManager to drive the
   * roller wheel animation in AdvantageScope. Sim use only.
   */
