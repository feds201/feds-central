...
public class TestingSubsystem extends SubsystemBase {
    ...
    @RobotAction(name = "1. Simple Pass", description = "A test that simply returns true to demonstrate a passing test.", order = 1)
    public boolean testSimplePass() {
        return true;
    }

    @RobotAction(name = "2. Simple Fail", description = "A test that simply returns false to demonstrate a failing test.", order = 2)
    public boolean testSimpleFail() {
        return false;
    }

    @RobotAction(name = "3. Exception Test", description = "A test that throws an exception to demonstrate error handling.", order = 3)
    public boolean testException() {
        throw new RuntimeException("This is a simulated exception for testing purposes.");
    }

    @RobotAction(name = "4. Timeout Test", description = "A test that sleeps longer than its timeout to demonstrate timeout handling.", order = 4, timeoutSeconds = 2.0)
    ...
    @RobotAction(name = "5. Diagnostic Alerts", description = "A test that logs info, warnings, and errors using DiagnosticContext.", order = 5)
    ...
    @RobotAction(name = "6. Data Profiling", description = "A test that generates a sine wave and cosine wave to demonstrate data profiling charts.", order = 6, timeoutSeconds = 5.0)
