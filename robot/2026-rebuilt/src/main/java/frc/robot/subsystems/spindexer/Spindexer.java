...
public class Spindexer extends SubsystemBase {
  ...
  public enum spindexer_state {
    RUN(Volts.of(8)), REVERSE(Volts.of(-8)), PREVERSE(Volts.of(-8)), PFORWARD(
        Volts.of(8)), STOP(Volts.of(0)), REVERSE_SHORT(Volts.of(-5));
  ...
  public Spindexer() {
    ...
    m_spindexerSysId =
        new SysIdRoutine(new SysIdRoutine.Config(Volts.of(0.5).per(Second), // default ramp (or Volts.of(x).per(Second) if you want custom)
            ...
            state -> SignalLogger.writeString("SysId_Spindexer_State",
  ...
  public void periodic() {
    Logger.recordOutput("Robot/Shooter/SpindexerOn",
        currentState != spindexer_state.STOP);
    Logger.recordOutput("Robot/Shooter/SpindexerState",
        currentState.toString());

    Logger.recordOutput("Robot/Spindexer/VelocityRPS",
        spindexerMotor.getVelocity().getValueAsDouble());
    Logger.recordOutput("Robot/Spindexer/TargetVolts",
        currentState.getVoltage().in(Volts));
  ...
  public double getSimSpindexerMotorVelocityRPS() {
    return spindexerMotor.getVelocity().getValue()
        .in(edu.wpi.first.units.Units.RotationsPerSecond);
  }
