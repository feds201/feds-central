...
public class CommandSwerveDrivetrain extends TunerSwerveDrivetrain
   ...
   *
   * @param drivetrainConstants Drivetrain-wide constants for the swerve drive
   * @param odometryUpdateFrequency The frequency to run the odometry loop. If unspecified or set to 0
   ...
   * @param odometryUpdateFrequency The frequency to run the odometry loop. If unspecified or set to 0
   *        Hz, this is 250 Hz on CAN FD, and 100 Hz on CAN 2.0.
   * @param odometryStandardDeviation The standard deviation for odometry calculation in the form [x,
  ...
  private void configureAutoBuilder() {
    try {
      ...
      AutoBuilder.configure(() -> getState().Pose, // Supplier of current robot pose
          ...
          (speeds,
              feedforwards) -> setControl(m_pathApplyRobotSpeeds
  ...
  public void periodic() {
     ...
     * perspective before, then we should apply it regardless of DS state. This allows us to correct the
     * perspective in case the robot code restarts mid-match. Otherwise, only check and apply the
     * operator perspective if the DS is disabled. This ensures driving behavior doesn't change until an
     * explicit disable event occurs during testing.
     */
  ...
  /**
   * Adjusts SwerveModuleConstants for MapleSim compatibility in simulation. Zeros encoder offsets,
   * disables motor inversions, and tunes steer PID. No-op on real robot.
   */
  private static SwerveModuleConstants<?, ?, ?>[] regulateModuleConstantsForSimulation(
    ...
    for (SwerveModuleConstants<?, ?, ?> mc : moduleConstants) {
      mc.withEncoderOffset(0).withDriveMotorInverted(false)
          .withSteerMotorInverted(false).withEncoderInverted(false)
          .withSteerMotorGains(
              new Slot0Configs().withKP(70).withKI(0).withKD(4.5).withKS(0)
                  .withKV(1.91).withKA(0).withStaticFeedforwardSign(
                      StaticFeedforwardSignValue.UseClosedLoopSign))
          .withSteerMotorGearRatio(16.0).withDriveFrictionVoltage(Volts.of(0.2))
  ...
  public boolean inOtherAlliance() {
    ...
    if (DriverStation.getAlliance().orElse(Alliance.Blue) == Alliance.Blue) {
      if (RobotMap.ShooterConstants.redAlliance
          .contains(getState().Pose.getTranslation())) {
        return true;
      }
    } else if (DriverStation.getAlliance()
        .orElse(Alliance.Blue) == Alliance.Red) {
      if (RobotMap.ShooterConstants.blueAlliance
          .contains(getState().Pose.getTranslation())) {
        return true;
      }
    }
  ...
  public boolean inNeutralZone() {
    if (ShooterConstants.neutralZone
        .contains(getState().Pose.getTranslation())) {
      return true;
    } else {
      return false;
  ...
  public boolean withinTrench() {
    ...
    Translation2d pose = getState().Pose.getTranslation();
    Double predictedX =
        pose.getX() + currentSpeeds.vxMetersPerSecond * pullOutTime;
    Double predictedY =
        pose.getY() + currentSpeeds.vyMetersPerSecond * pullOutTime;
