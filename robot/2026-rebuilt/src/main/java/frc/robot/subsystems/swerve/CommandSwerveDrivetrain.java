...
public class CommandSwerveDrivetrain extends TunerSwerveDrivetrain implements Subsystem {
  ...
  public CommandSwerveDrivetrain(SwerveDrivetrainConstants drivetrainConstants,
    ...
    super(drivetrainConstants, odometryUpdateFrequency, odometryStandardDeviation,
        visionStandardDeviation, regulateModuleConstantsForSimulation(modules));
    if (Utils.isSimulation()) {
      startSimThread();
    }
    configureAutoBuilder();
  ...
  private void configureAutoBuilder() {
    try {
      ...
      AutoBuilder.configure(() -> getState().Pose, // Supplier of current robot pose
          ...
          // case
          () -> DriverStation.getAlliance().orElse(Alliance.Blue) == Alliance.Red, this // Subsystem
                                                                                        // for
                                                                                        // requirements
      );
    } catch (Exception ex) {
  ...
  private static SwerveModuleConstants<?, ?, ?>[] regulateModuleConstantsForSimulation(
    ...
    for (SwerveModuleConstants<?, ?, ?> mc : moduleConstants) {
      mc.withEncoderOffset(0).withDriveMotorInverted(false).withSteerMotorInverted(false)
          .withEncoderInverted(false)
          .withSteerMotorGains(
              new Slot0Configs().withKP(70).withKI(0).withKD(4.5).withKS(0).withKV(1.91).withKA(0)
                  .withStaticFeedforwardSign(StaticFeedforwardSignValue.UseClosedLoopSign))
          .withSteerMotorGearRatio(16.0).withDriveFrictionVoltage(Volts.of(0.2))
          .withSteerFrictionVoltage(Volts.of(0.2)).withSteerInertia(KilogramSquareMeters.of(0.05));
