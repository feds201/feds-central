...
public class LedsSubsystem extends SubsystemBase {
  ...
  public LedsSubsystem() {
    ...
    // But periodic handles state change, so setting lastState to OFF calls applyState(IDLE) in
    // first
    // loop.
  }
  ...
  private static void applyState(LEDState state) {
    switch (state) {
      ...
      case INTAKING:
        m_leds.leds.SetAnimation(Animation.Blink).ForZone(ZONE_BACK67).WithColor(COLOR_GREEN)
      ...
      case SHOOTING:
        m_leds.leds.SetAnimation(Animation.Blink).ForZone(ZONE_ALL).WithColor(COLOR_WHITE)
            .WithDelay(Units.Milliseconds.of(50)).RunOnce(false);
        break;

      case CLIMBING:
        m_leds.leds.SetAnimation(Animation.RainbowRoll).ForZone(ZONE_ALL).WithColor(COLOR_WHITE)
            .WithDelay(Units.Milliseconds.of(10)).Reverse(false).RunOnce(false);
        break;

      case ERROR:
        m_leds.leds.SetAnimation(Animation.Blink).ForZone(ZONE_ALL).WithColor(COLOR_RED)
  ...
  private static void applyIdlePattern() {
    if (DriverStation.isDisabled()) {
      ...
      m_leds.leds.SetAnimation(Animation.Breathe).ForZone(ZONE_ALL).WithColor(COLOR_RED)
      ...
      m_leds.leds.SetAnimation(Animation.Breathe).ForZone(ZONE_ALL).WithColor(COLOR_FEDS_BLUE)
          .WithDelay(Units.Milliseconds.of(15)).RunOnce(false);
    }
  }
  ...
  public Command runStateCommand(LEDState state) {
    return startEnd(() -> setState(state), () -> setState(LEDState.IDLE)).ignoringDisable(true);
  }
