...
public class LedsSubsystem extends SubsystemBase {
  ...
  public static enum LEDState {
    OFF, IDLE, // Depends on Robot Mode (Disabled, Auto, Teleop)
  ...
  private static final Color COLOR_FEDS_BLUE =
      new Color(new Color8Bit(0, 168, 255)); // Team color
  ...
  public LedsSubsystem() {
    ...
    // But periodic handles state change, so setting lastState to OFF calls applyState(IDLE) in first
    // loop.
  }
  ...
  private static void applyState(LEDState state) {
    switch (state) {
      ...
      case INTAKING:
        m_leds.leds.SetAnimation(Animation.Blink).ForZone(ZONE_BACK67)
            .WithColor(COLOR_GREEN).WithDelay(Units.Milliseconds.of(200))
            .RunOnce(false);
        break;
      ...
      case SHOOTING:
        m_leds.leds.SetAnimation(Animation.Blink).ForZone(ZONE_ALL)
            .WithColor(COLOR_WHITE).WithDelay(Units.Milliseconds.of(50))
            .RunOnce(false);
        break;

      case CLIMBING:
        m_leds.leds.SetAnimation(Animation.RainbowRoll).ForZone(ZONE_ALL)
      ...
      case ERROR:
        m_leds.leds.SetAnimation(Animation.Blink).ForZone(ZONE_ALL)
            .WithColor(COLOR_RED).WithDelay(Units.Milliseconds.of(100))
            .RunOnce(false);
        break;
    }
  }
  ...
  private static void applyIdlePattern() {
    if (DriverStation.isDisabled()) {
      ...
      m_leds.leds.SetAnimation(Animation.Breathe).ForZone(ZONE_ALL)
      ...
      m_leds.leds.SetAnimation(Animation.Breathe).ForZone(ZONE_ALL)
          .WithColor(COLOR_FEDS_BLUE).WithDelay(Units.Milliseconds.of(15))
          .RunOnce(false);
    }
  }
  ...
  public Command runStateCommand(LEDState state) {
    return startEnd(() -> setState(state), () -> setState(LEDState.IDLE))
  ...
  public Command intakeSignal() {
    return runStateCommand(LEDState.INTAKING);
  }

  public Command hasGamePieceSignal() {
    return setStateCommand(LEDState.HAS_GAME_PIECE);
  } // Persist success

  public Command shootingSignal() {
    return runStateCommand(LEDState.SHOOTING);
  }

  public Command climbingSignal() {
    return runStateCommand(LEDState.CLIMBING);
  }

  public Command errorSignal() {
    return runStateCommand(LEDState.ERROR);
  }

  public Command resetLEDS() {
    return setStateCommand(LEDState.IDLE);
  }
