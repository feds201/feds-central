...
public class LedsSubsystem extends SubsystemBase {
  ...
  private static final Color COLOR_ORANGE =
      new Color(new Color8Bit(255, 100, 0));
  ...
  private static final Color COLOR_WHITE =
      new Color(new Color8Bit(255, 255, 255));
  private static final Color COLOR_YELLOW =
      new Color(new Color8Bit(255, 255, 0));
  private static final Color COLOR_GREEN = new Color(new Color8Bit(0, 255, 0));
  private static final Color COLOR_SCARLET =
      new Color(new Color8Bit(100, 14, 0));
  private static final Color COLOR_PURPLE =
      new Color(new Color8Bit(128, 0, 128));
  ...
  public void periodic() {
    ...
    if (m_currentStateLeft != m_lastStateLeft || dsModeChanged) {
      ...
      System.out.println("Switching LEDs to: " + m_currentStateLeft
          + " (DS Mode: " + currentDSMode + ")");
  ...
  private void applyState(RobotLEDState state) {
    switch (state) {
      ...
      case IDLE:
        ...
        break;
      case SHOOTING:
        m_leds.leds.SetAnimation(Animation.Blink).ForGroup(GR_ALL)
            .WithColor(COLOR_WHITE).WithDelay(Units.Milliseconds.of(100))
            .Reverse(false).RunOnce(false);
        break;

      case ERROR_LL:
        m_leds.leds.SetAnimation(Animation.Comet).ForZone(ZONE_LEFT)
            .WithColor(COLOR_LL).WithDelay(Units.Milliseconds.of(100))
            .RunOnce(false);
        break;

      case ERROR_CAN:
        m_leds.leds.SetAnimation(Animation.Comet).ForZone(ZONE_LEFT)
            .WithColor(COLOR_GREEN).WithDelay(Units.Milliseconds.of(400))
            .RunOnce(false);
        break;

      case ERROR_JAMMING:
        m_leds.leds.SetAnimation(Animation.Blink).ForZone(ZONE_LEFT)
            .WithColor(COLOR_SCARLET).WithDelay(Units.Milliseconds.of(200))
            .RunOnce(false);
        break;

      case ERROR_OTHER:
        m_leds.leds.SetAnimation(Animation.Blink).ForZone(ZONE_LEFT)
            .WithColor(COLOR_PURPLE).WithDelay(Units.Milliseconds.of(200))
            .RunOnce(false);
        break;

      case STARTUP_TEST:
        m_leds.leds.SetAnimation(Animation.Blink).ForZone(ZONE_LEFT)
            .WithColor(COLOR_ORANGE).WithDelay(Units.Milliseconds.of(10))
  ...
  private void applyLeds2State(DriveLEDState state) {
    switch (state) {
      case OFF:
        ...
        break;
      case NORMAL_DRIVE:
        m_leds.leds.SetAnimation(Animation.Chase).ForZone(ZONE_RIGHT)
            .WithColor(COLOR_GREEN).WithDelay(Units.Milliseconds.of(100))
            .Reverse(false).RunOnce(false);
        break;
      case HUB_DRIVE:
        m_leds.leds.SetAnimation(Animation.Confetti).ForZone(ZONE_RIGHT)
            .WithColor(COLOR_GREEN).WithDelay(Units.Milliseconds.of(10))
            .RunOnce(false);
        break;
      case FALCON_DRIVE:
        m_leds.leds.SetAnimation(Animation.Blink).ForZone(ZONE_RIGHT)
            .WithColor(COLOR_ORANGE).WithDelay(Units.Milliseconds.of(200))
            .RunOnce(false);
        break;
    }
  }
  ...
  private static void applyIDLE() {
    if (DriverStation.isDisabled()) {
      ...
      m_leds.leds.SetAnimation(Animation.Breathe).ForGroup(GR_ALL)
          .WithColor(COLOR_RED).WithDelay(Units.Milliseconds.of(20))
          .RunOnce(false);
    } else if (DriverStation.isAutonomous()) {
      // Auto: Rainbow indicating Autonomous mode
      m_leds.leds.SetAnimation(Animation.RainbowRoll).ForGroup(GR_ALL)
          .WithColor(COLOR_WHITE).WithDelay(Units.Milliseconds.of(10))
          .Reverse(false).RunOnce(false);
    } else if (DriverStation.isTeleop()) {
      m_leds.leds.SetAnimation(Animation.Blink)
          // blinking yellow in teleop
          .ForGroup(GR_ALL).WithColor(COLOR_YELLOW)
          .WithDelay(Units.Milliseconds.of(150)).RunOnce(false);
    } else if (DriverStation.isTest()) {
      // Fallback for any other state, solid white
      m_leds.leds.SetAnimation(Animation.Chase).ForGroup(GR_ALL)
          .WithColor(COLOR_YELLOW).WithDelay(Units.Milliseconds.of(100))
          .Reverse(false).RunOnce(false);
    }
  }
}
